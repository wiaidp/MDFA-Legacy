...
\paragraph{Proof of Proposition \ref{prop:asym.sigex}.}
  The filter error is $S_t - \widehat{S}_t$, and it suffices to 
  show that this is uncorrelated with $X_{t-j}$ for each $j \geq 0$.
  Thus we need to verify that
 $\mbox{Cov} [ S_t, X_{t-j} ] = \mbox{Cov} [ \widehat{S}_t, X_{t-j}]$.
 Because the signal and noise processes have mean zero, 
 the former quantity is $\Gamma^s_j$, the lag $j$ autocovariance 
 for the signal process.  The latter quantity is
\begin{align*}
   \EE [   { \{ \widehat{\Psi} * X \} }_t   \, X_{t-j}^{\prime} ] & = 
   \sum_{\ell = -\infty}^{\infty}
    \widehat{\Psi}_{\ell}  \, \EE [ X_{t-\ell} \, X_{t-j}^{\prime} ] \\
	& = \sum_{\ell = -\infty}^{\infty}  \widehat{\Psi}_{\ell}  \,  \Gamma^x_{j-\ell}  \\
	& = \frac{1}{2 \pi} \, \int_{-\pi}^{\pi}   \widehat{\Psi} (e^{-i \omega}) \, 
	\Theta^x (e^{-i \omega}) \, \Sigma_x \, { \Theta^x (e^{i \omega}) }^{\prime} \,
		 e^{i \omega j} \, d\omega \\
	& =  \frac{1}{2 \pi} \, \int_{-\pi}^{\pi}   
	 { [  \Theta^s (e^{-i \omega}) \, \Sigma_s \, {  \Theta^s (e^{i \omega}) }^{\prime}  \, 
    {  \Theta^x (e^{i \omega}) }^{- \prime}  ]}_0^{\infty}  \, 
    {  \Theta^x (e^{i \omega}) }^{\prime} \,
		 e^{i \omega j} \, d\omega   \\
& =  \frac{1}{2 \pi} \, \int_{-\pi}^{\pi}   
	   \Theta^s (e^{-i \omega}) \, \Sigma_s \, {  \Theta^s (e^{i \omega}) }^{\prime}  \, 
    		 e^{i \omega j} \, d\omega   \\
& -   \frac{1}{2 \pi} \, \int_{-\pi}^{\pi}   
	 { [   \Theta^s (e^{-i \omega}) \, \Sigma_s \, {  \Theta^s (e^{i \omega}) }^{\prime}  \, 
    {  \Theta^x (e^{i \omega}) }^{- \prime}  ]}_{-\infty}^{-1}  \, 
    {  \Theta^x (e^{i \omega}) }^{\prime} \,
		 e^{i \omega j} \, d\omega, 
\end{align*}
  where the last quantity follows from writing the power series
  ${ [  A(z)  ]}_0^{\infty} = A(z) - {[ A(z) ]}_{-\infty}^{-1}$, and
 cancelling terms.   Now observe that the second summand in the last 
 line involves only positive powers of $e^{i \omega}$
  in the integrand, because ${[ A(z) ]}_{-\infty}^{-1}$ only involves
  positive powers of $z^{-1}$; hence this second summand
 is zero for any $j \geq 0$.   The first summand has the signal's spectral
 density as the integrand, and hence equals $\Gamma^s_{j}$.
 This proves optimality.
To prove the second assertion,   the LPP solution of  (\ref{eq:GPPsoln}) can be written
\begin{align*}
    &  \sum_{\ell = 0 }^{\infty} \Psi_{\ell} \, z^{\ell} +
    \sum_{\ell = -1}^{-\infty} \Psi_{\ell}
 \,  { [  \Theta^x (z) ]}_{-\ell}^{ \infty  } \, z^{\ell} \, { \Theta^x (z) }^{-1} \\
 & = \sum_{\ell = 0 }^{\infty}  \Psi_{\ell} \, z^{\ell} +
 \sum_{\ell =-1 }^{-\infty} \Psi_{\ell}
 \,  \left(   \Theta^x (z) -   { [  \Theta^x (z) ]}_0^{-\ell-1}  
 \right) \, z^{\ell} \, { \Theta^x (z) }^{-1} \\
 & = \sum_{\ell = -\infty}^{\infty}  \Psi_{\ell} \, z^{\ell} -  
 \sum_{\ell =-1}^{-\infty}  \Psi_{\ell}
 \,     { [  \Theta^x (z) ]}_0^{-\ell-1}     \, z^{\ell} \, { \Theta^x (z) }^{-1}.
\end{align*}
  The first summand is $\Psi (z)$.    Noting that
\[
    { [  \Theta^x (z) ]}_0^{-\ell-1}  =  \sum_{k=0}^{-\ell-1}  \Theta^x_k \, z^k =
   \sum_{k \geq 0}  \Theta^x_{-\ell-1-k} \, z^{-\ell-1-k},
\]
for the second summand we have
\begin{align*}
&  \sum_{\ell = -1}^{-\infty}  \Psi_{\ell}   \,     { [  \Theta^x (z) ]}_0^{-\ell-1} 
\, z^{\ell}   \\
& = \sum_{\ell = -1}^{-\infty}   \Psi_{\ell}   \,   \sum_{k = 0}^{\infty} 
  \Theta^{x}_{-\ell-1-k} \, z^{-1-k} \\
 & =  \sum_{k = 0}^{\infty}    z^{-1-k} \, \sum_{\ell = -1}^{-\infty} \Psi_{\ell} \, 
   \Theta^x_{-\ell-1-k}   \\
...
\,  \Theta^x_{ -\ell-1-k}   \\
 ...
 \Theta^s (e^{-i \omega}) \, \Sigma_s \, {  \Theta^s (e^{i \omega}) }^{\prime}  \, 
    ...
    {  \Theta^x (e^{-i \omega}) }^{-1}
\, \sum_{\ell = -1}^{-\infty}  \Theta^x_{ -\ell-1-k}   \, e^{i \omega \ell} \, d\omega   \\
 & =  \sum_{k = 0}^{\infty}   z^{-1-k} \,  \frac{1}{2 \pi} \int_{-\pi}^{\pi}  
 \Theta^s (e^{-i \omega}) \, \Sigma_s \, {  \Theta^s (e^{i \omega}) }^{\prime}  \, 
    {  \Theta^x (e^{i \omega}) }^{- \prime} \, \Sigma_x^{-1} \, 
    {  \Theta^x (e^{-i \omega}) }^{-1}
\,  \sum_{h = 0}^{\infty}   \Theta^x_{h-k}   \, e^{-i \omega (h+1)} \, d\omega  \\
 & =   \sum_{k = 0}^{\infty}   z^{-1-k} \,  \frac{1}{2 \pi} \int_{-\pi}^{\pi}  
 \Theta^s (e^{-i \omega}) \, \Sigma_s \, {  \Theta^s (e^{i \omega}) }^{\prime}  \, 
    {  \Theta^x (e^{i \omega}) }^{- \prime} \, \Sigma_x^{-1} \,
    {  \Theta^x (e^{-i \omega}) }^{-1}
\,   \Theta^x (e^{-i \omega}) \, e^{-i \omega (k+1) }  \, d\omega  \\
 & =   \sum_{k = 0}^{\infty}   z^{-1-k} \,  \frac{1}{2 \pi} \int_{-\pi}^{\pi}  
 \Theta^s (e^{-i \omega}) \, \Sigma_s \, {  \Theta^s (e^{i \omega}) }^{\prime}  \, 
    {  \Theta^x (e^{i \omega}) }^{- \prime}  \, e^{-i \omega (k+1) } 
    \, d\omega   \, \Sigma_x^{-1}  \\
 & =   \sum_{k = 0}^{\infty}   z^{-1-k} \,  { \langle   \Theta^s ( e^{-i \omega}) \,
 \Sigma_s \, {  \Theta^s ( e^{i \omega} }^{\prime}  \, 
    {  \Theta^x (e^{i \omega}) }^{- \prime}   \rangle  }_{-(k+1) }  \, \Sigma_x^{-1}  \\
 & =   { [  \Theta^s (z) \, \Sigma_s \, {  \Theta^s (z^{-1}) }^{\prime}  \, 
    {  \Theta^x (z^{-1}) }^{- \prime}   ]}_{-\infty}^{-1}  \, \Sigma_x^{-1}.
\end{align*}
 On the other hand, the optimal concurrent filter can be expressed as 
\begin{align*}
 \widehat{\Psi} (z) & =  \left(      \Theta^s (z) \, \Sigma_s \, 
 {  \Theta^s (z^{-1}) }^{\prime}  \, 
    {  \Theta^x (z^{-1}) }^{- \prime}  -  
 { [   \Theta^s (z) \, \Sigma_s \, {  \Theta^s (z^{-1}) }^{\prime}  \, 
    {  \Theta^x (z^{-1}) }^{- \prime}  ]}_{-\infty}^{-1}  \right)  \,
    \Sigma_x^{-1} \, {  \Theta^x (z) }^{-1}  \\
 & = \Psi (z) -  { [   \Theta^s (z) \, \Sigma_s \, {  \Theta^s (z^{-1}) }^{\prime}  \, 
    {  \Theta^x (z^{-1}) }^{- \prime}  ]}_{-\infty}^{-1}   \, \Sigma_x^{-1} \, 
    {  \Theta^x (z) }^{-1}.
\end{align*}
...
\begin{Example} {\bf Petrol MB Replication.}
...
\begin{Exercise} {\bf Petrol MB Replication.}
...
HERE   Petrol figure
 ...
 We are interested in the performance of MDFA relative to the MB concurrent filter.
 We begin with a specification of the LLM exactly corresponding to the model fitted to the Petrol data, so that the MB concurrent filter
 $\widehat{\Psi} $ solves  the LPP.  (We refer to this as the null specification of the LLM.)
 We show that MDFA  (with appropriate constraints) can   replicate this optimal filter. 
 The optimal concurrent MB filter can be computed analytically -- see (\ref{eq:opt.conc.sigex}) of Appendix A for the 
stationary case.  If model mis-specification is anticipated, one could improve on this MB filter by using forecast
extensions from a fitted VAR (or co-integrated VAR), which is a flexible model class;  in contrast,   MDFA implicitly generates
forecast extensions nonparametrically.

 As with the VAR($1$) simulation
 we truncate the target MB filter to length $4001$ and generate a simulated LLM of length $4500$.   
 The simulations, together with the target trends (given by the WK trend filters), are displayed in  Figure \ref{fig:petrol.llmtrends}.

<<exercise_llmpetrol-mdfa,echo=True>>=
#
...
We apply MDFA in the manner described in Section 4.3,  where $\delta^S (z) = 1-z$ and $\delta^N (z) = 1$.
 By  extracting the bottom row (corresponding to $T=528$, the length of the petrol sample)
 of the matrix formula for the finite-sample filter (McElroy and Trimbur, 2015), we obtain a very close approximation to the MB concurrent filter,
  which we denote by $\widehat{\Psi} $.     Results are displayed in Figure \ref{fig:llm.nullmdfa} of the Appendix, with the target trend in black
 (these are the same black trend lines as displayed in Figure \ref{fig:petrol.llmtrends}, but here shown without the underlying simulation)
 and the MDFA real-time trend in dark grey; MDFA does a good job of tracking the real trend, although with some loss to smoothness in 
 addition to a lag effect -- this is to be expected of a concurrent filter, to some degree.  Indeed, the optimal concurrent filter
 (light grey) is exactly matched by the MDFA filter.
 The first column block of  Table \ref{tab:petrol.llm.mdfa} shows the in-sample MSE for 
 the two concurrent filters (MB versus MDFA), showing small discrepancies due to finite-sample effects,
   i.e., the MDFA filter replicates the optimal concurrent filter.
...
Next, we alter the specification to illustrate that MDFA can out-perform the MB concurrent filter.    We do this by substantially increasing the
 variability in the irregular, producing a noisier simulation -- now the MB concurrent will do too little smoothing.  The new irregular
...
\]
 We refer to this as the alternative LLM process. 
 The resulting simulation with trends
 is shown in Figure \ref{fig:llm.alttrends}.
Application of MDFA yields   the  trends plotted in Figure \ref{fig:llm.altmdfa} of the Appendix.  For the first series, MDFA tracks the
 MB trend better than the MB concurrent filter does;  the second block of columns in
 Table \ref{tab:petrol.llm.mdfa} shows the in-sample MSE for 
 the two concurrent filters, showing substantial improvements 
for MDFA ($26 \%$ and $22 \%$ reductions to MSE, respectively for the two series).
...
Finally, we conduct a comparison on the Petroleum data itself, recognizing that the LLM might be mis-specified.  We do not have the long samples
 available considered before, so we compute a series of MB trends based on the matrix formulas for signal extraction, based on the specified
 model, and delete the first and last five years of such trends.  The result is a trend that approximately corresponds to the output of the
 symmetric MB filter $\Psi $.  Taking the same MB filter as target, we compute the non-stationary MDFA solution to the LPP, and compare to the
 concurrent MB filter output.  The resulting trends are plotted in Figure \ref{fig:llm.datamdfa}.  The in-sample MSE is displayed in
 the final block of columns in  Table    \ref{tab:petrol.llm.mdfa}, showing a modest improvement for MDFA
 on the first series, and similar performance on the second series. 
