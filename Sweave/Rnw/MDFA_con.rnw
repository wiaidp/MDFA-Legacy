...
% HERE : need to rewrite in terms of initial values....using Wildi-McElroy (2016)
...
\[
  -{ \left[  \gamma_{\partial S} (z) \,  \delta^N (z^{-1}) \, {\delta^S (z)}^{-1}
     ...
     { \Theta (z) }^{-1} \delta (z).
\]
   This  frfcorresponds to a filter of the form $\Phi (L)  { \Theta (L) }^{-1} \delta (L)$,
   ...
   of $z$, which means that the fitler $\Phi (L)$ only involves negative powers of $L$.
