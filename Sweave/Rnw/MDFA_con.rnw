...
\end{equation}
 ...
 HERE:  what about doing cancellation?
 ...
 For computation, it is convenient to calculate the integrand given in the
  ...
  However, as there are only a finite number of such resolvable singularities,
   ...
   integrating over $\omega \in [-\pi, \pi]$ such that a neighborhood containing
    ...
    $ G \, {|\Delta (z) |}^{-2}$ are easily computed.  Alternatively,
    with a bit more effort one can determine the value of the integrand at $\omega$
    such that $\Delta (e^{-i \omega}) = 0$, and obtain an exact expression.
