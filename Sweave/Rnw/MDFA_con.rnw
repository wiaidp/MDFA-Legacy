...
f.mu <- mdfa.spectra(iden,iden,Sigma.mu,1000)
f.irr <- mdfa.spectra(iden,delta,Sigma.irr,1000)
...
for(i in 1:N)
{
  for(j in 1:N)
  {
    plot(ts(Re(trend.frf[i,j,]),frequency=1000),ylim=c(0,1),ylab="")
  }
}
@
...
for(i in 1:N)
{
  for(j in 1:N)
  {
    plot(ts(Re(sa.frf[i,j,]),frequency=1000),ylim=c(-.5,1),ylab="")
