...
\label{exam:constant.mean}
  ...
  \[
   \Lambda (B) \, \EE [ X_t] = \Lambda(B) \, \mu = \sum_j \lambda (j) \, \mu =
   \Lambda (1) \, \mu,
  \]
...
\label{exam:linear.mean}
 ...
 of a linear time trend.  Then it is required that  $\partial {\Lambda} (1) = 0$
  ...
  \[
   \Lambda (B) \, \EE [ X_t] = \Lambda(B) \, \mu \, t =   \mu \, \sum_j \lambda (j) \, (t-j)
...
\label{eq:mdfa-criterion-nonstat}
 D_{\Psi} (\vartheta, G) =     { \langle  \widetilde{\Lambda} (z)   \, 
 G \,  {\widetilde{\Lambda} (z) }^*   \rangle }_0
...
\]
To prove the second assertion, let $\Phi(z) = \Theta (z) / \Delta (z)$.
...
\vspace{.5cm}
 ...
 It is not clear how to compute (\ref{eq:wiener-hopf}) because of the presence of
  ${\Delta^S (z)}^{-1}$ in the bracket.  In Bell and Martin (2004) this issue was
  addressed by computing a partial fraction decomposition; here we take a similar
  approach, but adapted for multivariate time series.  The goal is to construct
  a matrix power series $D(z)= \sum_{k=0}^{\infty} d (k) z^k$ 
  and a strictly anti-causal $C(z^{-1}) = \sum_{k=-\infty}^{-1}  c (k) z^k$ such that
\[
    \gamma_{\partial S} (z) \,  \Delta^N (z^{-1}) \, {\Delta^S (z)}^{-1}
     { \Theta (z^{-1})}^{-1 \prime}  =  C ( z^{-1}) \, { \Theta (z^{-1})}^{-1 \prime}
     + D(z)  \, {\Delta^S (z)}^{-1}.
\]
 Once we have computed $D(z)$, we obtain a simple expression for the concurrent filter.
 
\begin{Corollary}
\label{cor:asym.sigex}
  Under the assumptions of Theorem \ref{thm:asym.sigex}, the optimal concurrent 
 filter  can be expressed as
 $\widehat{\Psi} (z) = D (z) \Sigma^{-1} \,
     { \Theta (z) }^{-1} \Delta^N (z)$, where $D(z)$ is given by
\begin{align*}
  D(z) &=    { \left[  \gamma_{\partial S} (z) \,  \Delta^N (z^{-1}) 
     { \Theta (z^{-1})}^{-1 \prime} \right] }_0^{\infty} \\
     & - \sum_{j=1}^{d_S} \delta^S (j) z^j {\left[ \gamma_{\partial S} (z) \, 
     \Delta^N (z^{-1}) \, { \Theta (z^{-1})}^{-1 \prime} \,
      { \Delta^S (z) }^{-1} \right] }_{-j}^{-1}. 
\end{align*}
\end{Corollary}

\paragraph{Proof of Corollary \ref{cor:asym.sigex}.}
