...
<<exercise_mdfa_var1.filtering,echo=False>>=
...
<<exercise_mdfa_bimdfa-udfa,echo=False>>=
...
<<exercise_mdfa_nowmdfa-udfa,echo=False>>=
strCaption <- paste0("Effect of lead and  inverse signal-to-noise ratio", 
                            "on MDFA filter MSE.")
print(xtable(critmdfa.mat, dec = 1,digits=rep(3,dim(critmdfa.mat)[2]+1),
      caption=strCaption,label="tab:critmdfa.mat"),
      size="footnotesize", #Change size; useful for bigger tables
      include.rownames=FALSE, #Don't print rownames
      include.colnames=FALSE, #We create them ourselves
      caption.placement="top", 
      hline.after=NULL, #We don't need hline; we use booktabs
      add.to.row = list(pos = list(-1,nrow(critmdfa.mat)),
...
strCaption <- paste0("Effect of lead and  inverse signal-to-noise ratio", 
                            "on DFA filter MSE.")
print(xtable(critmdfa.mat, dec = 1,digits=rep(3,dim(critudfa.mat)[2]+1),
      caption=strCaption,label="tab:critmdfa.mat"),
      size="footnotesize", #Change size; useful for bigger tables
      include.rownames=FALSE, #Don't print rownames
      include.colnames=FALSE, #We create them ourselves
      caption.placement="top", 
      hline.after=NULL, #We don't need hline; we use booktabs
      add.to.row = list(pos = list(-1,nrow(critudfa.mat)),
                        command = c(paste("\\toprule \n",
                                  "Model & $\\alpha$ & $\\beta$ & t-value & $R^2$\\\\\n", 
                                    "\\midrule \n"),
                                    "\\bottomrule \n") ) )
