...
\label{rem:wkfrf-coint}
 When co-integration is present the signal or noise processes can have reduced rank,
  and special case is needed in the calculation of the WK frequency response function.
 ...
 and there is no problem computing the frf. Next,  
  suppose that $\Delta^N (e^{-i \omega_*}) = 0$; this precludes 
  $\Delta^S (e^{-i \omega_*}) = 0$, because the signal and noise operators are assumed
  to be relatively prime.  

HERE  finish argument,  what about WH filter?
...
HERE   need to fix mdfa.whfrf too
