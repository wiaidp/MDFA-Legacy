...
#spec.0 <- spec.hat.sharp[2,2,]*spec.hat.sharp[3,3,]-spec.hat.sharp[3,2,]*spec.hat.sharp[2,3,]
#spec.1 <- spec.hat.sharp[1,2,]*spec.hat.sharp[3,3,]-spec.hat.sharp[1,3,]*spec.hat.sharp[3,2,]
#spec.2 <- spec.hat.sharp[1,2,]*spec.hat.sharp[2,2,]-spec.hat.sharp[1,3,]*spec.hat.sharp[2,3,]
#temp <- cbind(spec.1/spec.0,spec.2/spec.0)
# frf.psi.flat <- alpha %*% ??? +   frf.psi.sharp - frf.coint

lp.mdfa.sharp <- mdfa.unconstrained(frf.psi.sharp,spec.hat.sharp,q-1)
...
lp.mdfa.filter <- array(t(R.mat %*% t(matrix(lp.mdfa.sharp[[1]],nrow=N))
                             + Q.mat),c(N,N,q))
