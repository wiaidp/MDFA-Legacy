...
\]
Clearly $\Psi (1)$ has reduced rank $n-r < n$,
...
\]
 We proceed to derive $\beta$ and $\alpha$ from the Cholesky decomposition of
  $\Sigma_S$.  We can write $\Sigma_S = L L^{\prime}$ for $n \times n-r$-dimensional
   ...
   (McElroy, 2017),   where the rank configuration of $\Sigma_S$ determines which 
    columns are excluded. In fact, there exist selection matrices $P$ and $M$
    that are subsets of the identity matrix (i.e., $[P \, M]$ is a permutation
     matrix), having dimensions $n \times n-r$ and $n \times r$ respectively,
    and such that $\widetilde{L} \, P = L$.  
 ...
 So the action of $P$ is  to remove $r$ of the columns of $\widetilde{L}$.
  Then $H = P P^{\prime}$ is a diagonal matrix    with $n-r$ ones
    and $r$ zeroes on the diagonal, with a zero corresponding to an omitted column
    in $L$.    Because $H$ is idempotent, this discussion shows that we
    have $\Psi (1) = \widetilde{L} H$.   Due to the definition of $P$ and $M$,
    it follows that $M^{\prime} P = 0$, which suggests setting $\alpha = M$
    and  $\beta = \widetilde{L}^{-\prime} M$, which shows that
    \[
    \Psi (1) \, \alpha = \widetilde{L} P P^{\prime} M = 0 \qquad 
    \mbox{and} \qquad 
  \beta^{\prime} \, \Psi (1) = M^{\prime} \widetilde{L}^{-1} \widetilde{L} P P^{\prime} = 0.
  \]
 Note that we could also define $\alpha = M \, G$ for any $r \times r$ matrix $G$;
 in particular, we could scale each column of $M$ in the definition of $\alpha$.
   But with our primal definition of $\alpha$ and $\beta$, we find that
\begin{equation}
\label{eq:co-int.constraint-llm}
 \alpha \beta^{\prime} = M \, M^{\prime} \widetilde{L}^{-1} = (1_n - H) \widetilde{L}^{-1}.
\end{equation}
