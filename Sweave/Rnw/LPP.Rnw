...
This chapter develops the basic framework of the book by
discussing the so-called Linear Prediction Problem (LPP).
We begin with a rapid introduction to vector time series,
which mainly serves to set our notation -- it is presumed
that the reader already has familiarity with these topics --
and then proceed to the main discussion of LPP, and how
 models can be fitted through an LPP criterion.
...
\]
 ...
 The advantage of the spectral representation is that it quickly facilitates
