...
\chapter{Customization for the Multivariate DFA}\label{atsm_sec}
...
Chapter ? discussed the multivariate DFA, giving the treatment of stationary, non-stationary, and co-integrated processes.  It was shown how filter constraints were related to non-stationary properties of the processes, and could be imposed in order to ensure that the filter error was stationary; and also how co-integration constraints could also achieve this.  Here we generalize the ATS-trilemma of stationary univariate DFA to the multivariate case, which allows for customization of multivariate filters.


\section{Multivariate Customization}
We begin with the M-DFA MSE and 
...
\section{Examples of M-DFA Customization}

HERE: use multivariate HP as target filter
