...
ts.plot(scale(filter_mat,center=F,scale=rep(T,ncol(filter_mat))),col=colo)
...
ts.plot(scale(y_mat[1:min(len-L,1000),],center=F,scale=rep(T,ncol(y_mat))),col=colo,xlab="")
abline(h=0)
for (i in 1:ncol(filter_mat))
  mtext(colnames(filter_mat)[i],col=colo[i],line=-i)
...
ts.plot(scale(y_mat[1:1000,],center=F,scale=rep(T,ncol(y_mat))),col=colo,xlab="")
abline(h=0)
for (i in 1:ncol(filter_mat))
  mtext(colnames(filter_mat)[i],col=colo[i],line=-i)
dev.off()
