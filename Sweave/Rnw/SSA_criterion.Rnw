...
The optimization criterion is derived in section \ref{zc} with a discussion of robusteness and extensions of the basic methodological framework (BMF); solutions of the criterion are proposed in section \ref{theorem_SSA} with a discussion of boundary and singular cases, numerical aspects as well as the sample distribution; section \ref{examples} illustrates various applications including ordinary forecasting, extensions to more general processes, resilience against departures from the BMF, smoothing and 'un-smoothing', convolution and deconvolution, Plancherel-identity and amplitude functions, a smoothness-timeliness dilemma, multiplicity and uniqueness features as well as a fully fleshed-out singular case. All empirical examples are replicable by an open source R-package (include link to Github). Finally, section \ref{conclusion} concludes by summarizing our main findings. 
...
\begin{Proposition}
Under the BMF let $\hat{z}_{t+\delta}=\sum_{k=0}^{L-1}\gamma_{k+\delta}\epsilon_{t-k}=\boldsymbol{\gamma}_{\delta}'\boldsymbol{\epsilon}_{t}$ designate the classic MSE-predictor. Then $z_t$ in criterion \ref{crit1} can be replaced by $\hat{z}_{t+\delta}$.
\end{Proposition}
Proof\\
...
Consider the SSA optimization problem \ref{crit1} under the BMF and the following set of regularity assumptions:
\begin{enumerate}
...
\textbf{Remarks}\\
Extensions to autocorrelated $x_t$ are straightforward, see section \ref{ext_stat} for background and sections \ref{example_autocor} to \ref{conv_amp} for illustration. Also, Gaussianity is not required in the derivation of the above proof because the SSA-criterion \ref{crit1} relies solely on correlations. The Gaussian hypothesis is needed when  establishing formal links between correlations and sign-accuracy or holding-time concepts but  $y_t$ or $z_t$ can be nearly Gaussian even if $\epsilon_t$ isn't, see also section \ref{resil} for illustration. More generally, the proof applies to arbitrary symmetric $\tilde{\mathbf{M}}$ with pairwise different eigenvalues $\tilde{\lambda}_i$ so alternatives to the holding-time  constraint, with a similar symmetry-structure, could be addressed. Note also that the limiting cases $|\nu|\to\infty$ correspond to the degenerate case $\mathbf{b}\propto\boldsymbol{\gamma}_{\delta}$ since then $\boldsymbol{\nu}/\nu\to-\mathbf{I}$. Equivalently, the difference-equation \ref{ar2} morphs into an identity, up to arbitrary scaling. Another interesting limiting case occurs when $\mathbf{b}\to\mathbf{v}_i$ since then $D\boldsymbol{\gamma}_{\delta}=\boldsymbol{\nu}\mathbf{b}\to(2\lambda_i-\nu)\mathbf{v}_i$. If $\boldsymbol{\gamma}_{\delta}$ has complete spectral support, then we conclude that $D\to 0$ or, equivalently $\lambda_2\to\infty$ which would imply $i=1$ or $i=L$ (otherwise the criterion cannot be overruled by the constraint, since the intersection of unit-sphere and holding-time ellipse is an $L-2\geq 1$ dimensional space). On the other hand, if $\boldsymbol{\gamma}_{\delta}\to\mathbf{v}_i$ is allowed to have asymptotically incomplete spectral support and if $D\not\to 0$ then $D\boldsymbol{\gamma}_{\delta}=\boldsymbol{\nu}\mathbf{b}\to(2\lambda_i-\nu)\mathbf{b}$ so that $\mathbf{b}\propto \boldsymbol{\gamma}_{\delta}$ (degenerate case) and $\rho_1\to\lambda_i$ (from the previous limiting case we then infer $|D|\to\infty)$. We conclude that if $\mathbf{b}\to\mathbf{v}_i$ then either $i\in\{1,L\}$ and $|\rho_1|\to\rho_{max}(L)$ (boundary cases of admissibility) or $\mathbf{b}\propto\boldsymbol{\gamma}_{\delta}\to \mathbf{v}_i$ and $\rho_1\to\lambda_i$ for any $i\in\{1,...,L\}$ (degenerate case with asymptotically incomplete spectral support). The case of incomplete support is addressed in the following corollary. 
...
SSA_obj<-SSA_func(L_short,delta,grid_size,gamma_target,rho0,with_negative_lambda,xi)
# Is the same as
...
SSA_obj<-SSA_func(L_long,delta,grid_size,gamma_target,rho0,with_negative_lambda,xi)
...
\subsection{Extension to Autocorrelated Processes}\label{example_autocor}
...
ts.plot(xi)
# MSE and Lag-by-one benchmarks as applied  to white noise i.e. convolution of MA(3)-filter and ARMA-filter (would require deconvolution if applied to xt)
...
where $x_t=\Sexpr{ar1}x_{t-1}+\epsilon_t+\Sexpr{ma[1]}\epsilon_{t-1}+\Sexpr{ma[2]}\epsilon_{t-2}$ is a stationary and invertible ARMA(1,2)-process with MA-inversion or Wold-decomposition $x_t=\sum_{k=0}\xi_k\epsilon_{t-k}$. %, displayed in figure \ref{filt_coef_example1_arma} (black lines in bottom left and right panels). 
The convolutions $(b\cdot\xi)_j$, $j=0,...,L-1$, proposed in section \ref{ext_stat} and obtained by applying SSA to $\epsilon_t$ of the MA-inversion of $x_t$, are displayed in the bottom panels of fig.\ref{filt_coef_example1_arma}.
<<label=z_box_plot_pure_mba_2.pdf,echo=FALSE,results=tex>>=
file = "filt_coef_example1_arma"
cat("\\begin{figure}[H]")
cat("\\begin{center}")
cat("\\includegraphics[height=3in, width=6in]{", file, "}\n",sep = "")
cat("\\caption{SSA arbitrarily scaled. All lags (left panel) and first ten lags (right panel). Filters as applied to ARMA-process (upper panels) vs. filters as applied to white noise of MA-inversion (bottom panels). ", sep = "")
cat("\\label{filt_coef_example1_arma}}", sep = "")
cat("\\end{center}")
cat("\\end{figure}")
@
While $(b\cdot\xi)_j$ are derived from theorem \ref{lambda}, the 'ordinary' $\mathbf{b}$, as applied to $x_t$, can be obtained iteratively, by inversion or deconvolution of $(b\cdot\xi)_j$:
...
cat("\\begin{center}")
cat("\\includegraphics[height=3in, width=6in]{", file, "}\n",sep = "")
cat("\\caption{Amplitude functions of SSA AR(2)-filters as applied to ARMA-process (left) and to white noise  (right) and amplitude of ARMA-target (violet)", sep = "")
...
Wildi (2023) applies the SSA-approach to (the log-returns of) a broadly diversified equity index, the Standard and Poors 500, as well as  to industrial production indices of a selection of countries with long and consistent sample histories. Empirical and theoretical holding-times match virtually perfectly for the financial time series, despite volatility clustering, non-vanishing mean (drift) and extreme observations during financial and pandemic crises. Discrepancies observed in the case of the macro-indicators were attributable to autocorrelation and could be alleviated by the extension to autocorrelated processes illustrated in the above sections. 
<<label=init,echo=FALSE,results=hide>>=
len<-100000
...
@
We here complement these findings by an application of SSA to white noise $x_t=\epsilon_t$ where $\epsilon_t$ is t-distributed with degrees of freedom ranging from $df=\Sexpr{df_vec[1]}$ (heavy tails) to $df=\Sexpr{df_vec[length(df_vec)]}$ (nearly Gaussian). We then compare empirical holding-times, i.e. length of filter-outputs divided by number of zero-crossings, to theoretical holding-times based on long samples of size \Sexpr{as.integer(len)} of $\epsilon_t$, see table \ref{emp_ht}.
<<label=ats_mba_2,echo=FALSE,results=tex>>=
...
ma_len<-20
target<-rep(1,ma_len)
forecast_horizon<-ma_len/2
L_long<-50
# MA-inversion of ARMA-target
ht_target<-compute_holding_time_func(target)$ht
# We assume that input is white noise
xi<-NULL
# Impose nearly rho_max (rho_max=L_long+1)
ht<-ht_first<-50
rho0<-compute_rho_from_ht(ht)$rho
...
SSA_obj<-SSA_func(L_long,delta,grid_size,gamma_target,rho0,with_negative_lambda,xi)
bk_mat<-SSA_obj$bk_mat
compute_holding_time_func(bk_mat)$ht
colnames(bk_mat)<-paste("SSA(",round(ht,2),",",forecast_horizon,")",sep="")
colnames(bk_mat)<-paste("SSA(",round(ht,2),",",forecast_horizon,")",sep="")

# Same as above but longer filter
L_very_long<-5*L_long

SSA_obj<-SSA_func(L_very_long,delta,grid_size,gamma_target,rho0,with_negative_lambda,xi)

bk_mat<-cbind(c(bk_mat,rep(0,L_very_long-L_long)),SSA_obj$bk_mat)
...
@
Typically, the coefficients of $\mathbf{b}$ decay towards zero at a rate determined by $\boldsymbol{\gamma}_{\delta}$ and the holding-time constraint $\rho_1$ or $ht_1$. Figure \ref{filt_coef_example4} displays two SSA-predictors with the same holding-time $\Sexpr{round(ht,1)}$ but with different filter-lengths $L=\Sexpr{L_long}$ (blue) and $L=\Sexpr{L_very_long}$ (red). The shorter filter is subject to a near unit-root because the holding-time approaches the upper limit $\rho_{max}(50)$ for that filter length: its coefficients do not follow an exponential-law at higher lags. In contrast, increasing the filter length leads to an exponentially decaying pattern at higher lags (red line, right panel). In applications, the unit-root case typically occurs when $L$ and $ht_1$ are mismatched: in this case, filter performances are affected because the holding-time constraint 'overrules' the proper target-fitting: the Lagrangian parameter $\lambda_2$ in the proof of theorem \ref{lambda} grows unboundedly. In such a case, an increase of the filter-length $L$ unleashes degrees of freedom and improves MSE-performances.   
<<label=init,echo=FALSE,results=hide>>=
...
colo<-c("blue","red","violet","black","green")
par(mfrow=c(1,2))
mplot<-as.matrix(bk_mat[1:L_long,1])
plot(mplot[,1],main="Shorter filter",axes=F,type="l",xlab="Lag-structure",ylab="filter-weights",ylim=c(min(mplot),max(na.exclude(mplot))),col=colo[1],lwd=2)
mtext(colnames(mplot)[1],col=colo[1],line=-1)
axis(1,at=1:nrow(mplot),labels=-1+1:nrow(mplot))
axis(2)
box()
...
mplot<-bk_mat
plot(mplot[,1],main="Short vs. long filters",axes=F,type="l",xlab="Lag-structure",ylab="filter-weights",ylim=c(min(mplot),max(na.exclude(mplot))),col=colo[1],lwd=2)
mtext(colnames(mplot)[1],col=colo[1],line=-1)
for (i in 2:ncol(mplot))
{
#  lines(mplot[,i]-ifelse(i==ncol(mplot),0.3,0),col=colo[i],lwd=2)
...
cat("\\begin{center}")
cat("\\includegraphics[height=3in, width=6in]{", file, "}\n",sep = "")
cat("\\caption{SSA coefficients: identical holding-time ht=50 but different filter lengths L=50 (blue) and L=250 (red).", sep = "")
...
ts.plot(scale(filter_mat,center=F,scale=rep(T,ncol(filter_mat))),col=colo_all)

#   Filter data
filter_obj<-SSA_filter_func(filter_mat,L,series)

y_mat=filter_obj$y_mat
ts.plot(scale(y_mat,center=F,scale=rep(T,ncol(y_mat))))
...
select_vec<-1:3
mplot<-scale(bk_mat[,select_vec],center=F,scale=rep(T,length(select_vec)))
...
mat_coef_hp<-scale(cbind(hp_trend,modified_hp_gap),center=F,scale=c(T,T))
mat_coef_SSA<-scale(bk_mat[,select_vec],center=F,scale=rep(T,length(select_vec)))
...
pdf(file = paste(path.out,file,sep=""), paper = "special", width = 6, height = 6)
par(mfrow=c(2,2))
mplot<-scale(mat_amp_hp,center=F,scale=rep(T,ncol(mat_amp_hp)))
...
mplot<-scale(mat_amp_SSA,center=F,scale=rep(T,ncol(mat_amp_SSA)))
...
#   Filter data
  ...
  ts.plot(scale(y_mat,center=F,scale=rep(T,ncol(y_mat)))[,c("SSA(12,18)","HP trend")],col=c("blue","brown"))
  abline(h=0)
...
We propose a novel  SSA-criterion which emphasizes sign accuracy and zero-crossings of the predictor subject to a holding-time constraint. Under the Gaussian assumption, the classic MSE-criterion is equivalent to  unconstrained SSA-optimization: in the absence of a holding-time constraint and down to an arbitrary scaling nuisance. We argue and demonstrate that the proposed concept is resilient against various departures of the Gaussian assumption. Moreover, %Resilience against departures of the Gaussian hypothesis, in terms of  stylized facts of financial times series, has been verified by comparing  expected holding times with empirical means based on the S$\&$P500-index as well as BTC (crypto-currency). While heavy tails or autocorrelation can generate biases, the latter problem could be corrected by the proposed extension of our approach to stationary processes.   
%Notwithstanding, 
the approach is interpretable and appealing %beyond the promoted sign-accuracy perspective, in part 
due to its actual simplicity and because the criterion merges relevant concepts of prediction in terms of sign accuracy, MSE, and smoothing requirements. The business-cycle case illustrates application to an existing benchmark in view of modifying predictors according to particular research priorities. In this context, timeliness and smoothness of real-time designs can be controlled in an effective and interpretable way. %The example also illustrates that timeliness (advance or retard at the zero-line) and smoothing-capability (spread between consecutive crossings) of SSA-designs can be improved both, at once, when compared to established benchmarks. %These somehow intriguing observations hint towards existence of a richer tradeoff, a trilemma, which reconciles particular empirical findings  in a common formal framework. 
...
\bibitem{} Wildi, M. (2023) submitted JBCY
