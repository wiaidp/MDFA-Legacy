...
\begin{eqnarray*}
E[z_{it+\delta}y_{it}]&=&\boldsymbol{\gamma}_{i\cdot\delta}'\tilde{\mathbf{I}}\mathbf{b}_{i}\\
E[z_{it+\delta}^2]&=&\boldsymbol{\gamma}_{i\cdot\delta}'\tilde{\mathbf{I}}\boldsymbol{\gamma}_{i\cdot\delta}\\
E[y_{it}^2]&=&\mathbf{b}_{i}'\tilde{\mathbf{I}}\mathbf{b}_{i}\\
E[y_{it-1}y_{it}]&=&\mathbf{b}_{i}'\tilde{\mathbf{M}}\mathbf{b}_{i}
\end{eqnarray*}
where $\tilde{\mathbf{I}}=\boldsymbol{\Sigma}\otimes\mathbf{I}_{L*L}$ and $\tilde{\mathbf{M}}=\boldsymbol{\Sigma}\otimes\mathbf{M}$ are the $nL*nL$-dimensional Kronecker products of the $n*n$ dimensional variance $\boldsymbol{\Sigma}$ and the $L*L$ dimensional identity  $\mathbf{I}_{L*L}$ and $\mathbf{M}$, respectively.


\subsection{Sign-Accuracy, MSE and Holding-Time: Multivariate Extension}\label{mse_sa_zc}
...
Under the BMF the sign accuracy  criterion can be stated as
\begin{eqnarray}\label{opt_crit}
\max_{\mathbf{b}_i}\rho(y_i,\hat{{z}}_{i\delta},\delta)
\end{eqnarray}
where 
\[
\rho(y_i,\hat{{z}}_{i\delta},\delta)=\frac{\boldsymbol{\gamma}_{i\cdot\delta}'\tilde{\mathbf{I}}\mathbf{b}_{i}}{\sqrt{\mathbf{b}_{i}'\tilde{\mathbf{I}}\mathbf{b}_{i}}\sqrt{\boldsymbol{\gamma}_{i\cdot\delta}'\tilde{\mathbf{I}}\boldsymbol{\gamma}_{i\cdot\delta}}}
\] 
is the correlation between $y_{it}$ and the MSE-estimate $\hat{{z}}_{it\delta}$ of $z_{it+\delta}$. \end{Proposition}
...
\end{eqnarray*}
where $\rho(y_i,z_i,\delta)$ is the correlation between $y_{it}$ and $z_{it+\delta}$. The first equality follows from the Gaussian assumption and the second equality is obtained from orthogonal projection of $z_{it+\delta}$ onto the space spanned by $x_{jt-k}$, $j=1,...,n$, $0\leq k\leq L-1$ ($y_{it}$ does not depend on $\epsilon_{jt-k}$ for $k\notin\{0,...,L-1\}$). The proof then follows from strict monotonicity of the non-linear transformation. \\
...
%The MSE-estimate  $\mathbf{b}=\boldsymbol{\gamma}_{\delta}:=(\gamma_{\delta},...,\gamma_{\delta+L-1})'$ is a solution of \ref{opt_crit} 
We then infer that SA and MSE are equivalent criteria, at least down to an arbitrary scaling of $y_{it}$ and conditional on the Gaussian assumption.\\
...
\end{eqnarray}
where $\rho(y_i,y_i,1)=\displaystyle{\frac{\mathbf{b}_i'\mathbf{\tilde{M}}\mathbf{b}_i}{\mathbf{b}_{i}'\tilde{\mathbf{I}}\mathbf{b}_{i}}}$ is the lag-one autocorrelation of $y_{it}$. 
\end{Proposition}

A proof is provided by Kedem (1986). We can now formalize the concept of 'smoothness' of a predictor $y_{it}$ by constraining $\mathbf{b}_i$ such that
\begin{equation}\label{ht_const}
...
\right\}
\end{eqnarray}
This optimization problem is called \emph{simple sign-accuracy} or SSA-criterion: simplicity here refers to the elementary structure of the predictor, as derived in theorem \ref{lambda}. We allude  to solutions of this criterion by the acronym SSA or SSA($ht_i,\delta$) or SSA($\rho_i,\delta$) to stress the dependence of the predictor on the pair of hyper-parameters, see section \ref{time_smooth} for reference. The SSA-criterion merges MSE, sign accuracy and smoothing requirements in a flexible and consistent way. Departures from the Gaussian assumption can be accommodated in the sense that $y_{it}$ or $z_{it}$ can be 'nearly Gaussian' even if $x_{it}=\epsilon_{it}$ is not, due to the central limit theorem, see Wildi (2023 a) for an application to financial data (equity index), Wildi (2023 b) for an application to data with heavy-tails in a univariate framework and section \ref{resil}. Finally, the  criterion remains appealing outside of a strict holding-time or zero-crossing perspective by complementing the classic predictor with a generic smoothing constraint addressing the lag-one autocorrelation function (acf).        
<<label=init,results=hide>>=
# Purposes
# 0. Use Gauss or student-t (if skewed then one has to shift by mean)
# 1. Check that MSE/correlation has same sign-accuracy as logit, in-sample
...
\subsection{Extension to Stationary Processes: Multivariate Case}\label{ext_stat}
...
\end{eqnarray*} 
where $(\boldsymbol{\Gamma}\cdot\boldsymbol{\Xi})_k =\sum_{m\leq k}\boldsymbol{\Xi}_{k-m}\boldsymbol{\Gamma}_m$ and $(\mathbf{B}\cdot\boldsymbol{\Xi})_j=\sum_{n=0}^{\min(L-1,j)}\boldsymbol{\Xi}_{j-n} \mathbf{B}_n $ are convolutions of the sequences $\boldsymbol{\Gamma}_k$ and $\mathbf{B}_j$ with the Wold-decomposition $\boldsymbol{\Xi}_m$ of $\mathbf{x}_t$. Let $(\boldsymbol{\Gamma}\cdot\boldsymbol{\Xi})_{klm}$ and $(\mathbf{B}\cdot\boldsymbol{\Xi})_{jlm}$ denote the $lm$ element of the corresponding matrices, assume that $\delta\geq 0$ (for- and nowcasting\footnote{Similar but notationally more cumbersome expressions would be obtained for $\delta<0$ (backcast) which are skipped here.}) and denote
\begin{eqnarray*}
...
\end{eqnarray}
which can be solved for $(\mathbf{b}\cdot\boldsymbol{\xi})_i$, see theorem \ref{lambda_mult}. We refer to textbooks for a derivation of the multivariate Wold decomposition based on a finite sample $x_1,...,x_T$, see e.g. Brockwell and Davis (1993): worked-out examples are provided in sections \ref{example_autocor}, \ref{smooth_unsmooth} and \ref{conv_amp}.  Finally, for notational convenience we henceforth rely on the BMF, acknowledging that straightforward modifications would apply in the case of autocorrelation.     
  %From an empirical perspective, we argue that growth-rates of a wide range of economic time series are in accordance with our simplifying assumption, see e.g. the so-called 'typical spectral shape' of an economic variable in Granger (1966). %To conclude, we note that the procedure could be extended to non-stationary integrated processes. % and its utility would be questionable in the context of suitably transformed  data, typically differences or log-returns, at least if the transformation does not impede the analysis.   % assumption in terms of  conditional heteroscedasticity (vola-clustering) or so-called 'fat tails' (large kurtosis, outliers) is analyzed in section \ref{robustness_SSA}.
...
\section{Solution of the Multivariate SSA-Criterion}\label{theorem_SSA}
...
\sum_{m=1}^n\sigma_{jm}(b_{imk+1}-\nu b_{imk}+b_{imk-1})&=&D_i\sum_{m=1}^n\sigma_{jm}\gamma_{imk+\delta}~,~1\leq k\leq L-2\\
\sum_{m=1}^n\sigma_{jm}(b_{im1}-\nu b_{im0})&=&D_i\sum_{m=1}^n\sigma_{jm}\gamma_{im\delta}~,~k=0\nonumber\\
\sum_{m=1}^n\sigma_{jm}(-\nu b_{imL-1}+b_{imL-2})&=&D_i\sum_{m=1}^n\sigma_{jm}\gamma_{imL-1+\delta}~,~k=L-1\nonumber
\end{eqnarray}
...
For a proof of the last assertion (uniqueness) we note that \ref{specdecb_mult} has the same structure as in the univariate case and therefore, we can rely on the proof in theorem \ref{lambda} for deriving uniqueness.\\
...
\end{eqnarray*}
would lead to Kroneckerproducts $\mathbf{M}\otimes\boldsymbol{\Sigma}$ and $\mathbf{I}\otimes\boldsymbol{\Sigma}$ and the proof would remain unaffected. Furthermore, while the arbitrary scaling term is the same across all SSA sub-filters $\mathbf{b}_{ij}$, $j=1,...,n$ of $\mathbf{b}_i$ for fixed $i$, the scale of $\mathbf{b}_i$ could change across targets $z_{it}$, $i=1,...,n$. Finally, we did not treat the  case of a target $\boldsymbol{\gamma}_{i\cdot\delta}$ with incomplete spectral support mainly because the corresponding singular case is irrelevant for typical applications. Wildi (2023 b) derives a formal solution for univariate prediction problems. %Also, \ref{ssa_mult} suggests that the mutivariate SSA-filter assigns equal weight $\frac{1}{(2\lambda_{k}-\nu_i)}$ to the slicing or aggregate $\sum_{j=1}^n w_{ikj}\mathbf{\tilde{v}}_{kj}$ across spectral-weights of the vectors $\mathbf{\tilde{v}}_{kj}$ corresponding to $\boldsymbol{\epsilon}_{ij\cdot}=(\epsilon_{i1\cdot},...,\epsilon_{in\cdot})$. Finally, the orthormal basis $\mathbf{\tilde{v}}_{ik}$ and its adjoined subspace $\tilde{\mathbb{R}}_i^{L}$ of $\mathbb{R}^{nL}$ depend on $i$.\\
...
\subsection{Application to a Multivariate VAR(1)-Process}
...
{  
  ...
  SSA_obj<-SSA_func(L,delta,grid_size,gamma_target,rho0,with_negative_lambda,xi,lower_limit_nu,Sigma)
...
#-------------------------    
# Save  
  save(file=paste(path.result,"bk_mat_var1",sep=""),bk_mat)
...
{
  load(paste(path.result,"bk_mat_var1",sep=""))
...
\[\left(\begin{array}{c}z_{1t}\\z_{2t}\end{array}\right)=\left(\begin{array}{cc}\Sexpr{A[1,1]}&\Sexpr{A[1,2]}\\\Sexpr{A[2,1]}&\Sexpr{A[2,2]}\end{array}\right)\left(\begin{array}{c}z_{1t-1}\\z_{2t-1}\end{array}\right)+\left(\begin{array}{c}\epsilon_{1t}\\\epsilon_{2t}\end{array}\right)~~~,~~~\Sigma_{\epsilon}=\left(\begin{array}{cc}\Sexpr{round(Sigma[1,1],2)}&\Sexpr{round(Sigma[1,2],2)}\\\Sexpr{round(Sigma[2,1],2)}&\Sexpr{round(Sigma[2,2],2)}\end{array}\right)\]
where $\Sigma_{\epsilon}$ is the variance-covariance matrix of $\boldsymbol{\epsilon}_t=(\epsilon_{1t},\epsilon_{2t})'$. We compute a one-step ahead ($\delta=1$) SSA-predictor with holding-times  $ht_1=\Sexpr{ht_vec[1]}$, $ht_2=\Sexpr{ht_vec[2]}$. Weights of SSA- and MSE-predictors are displayed in fig.\ref{filt_coef_var1} and performances in terms of holding-times and criterion-values  are summarized in table \ref{perf_var1}. Note that SSA-criteria measure correlations with the MSE-predictors, recall proposition \ref{propo4}. The table illustrates that the SSA-predictor of the first target $z_{1t+\delta}$ has a shorter holding-time than the MSE-predictor: the predictor 'unsmooths' the target while performing forecasting. The SSA-predictor of the second target $z_{2t+\delta}$ has a longer holding-time than the MSE-predictor: the predictor 'smooths' the target while performing forecasting.     
<<label=init,echo=FALSE,results=hide>>=
...
cat("\\begin{center}")
cat("\\includegraphics[height=3in, width=6in]{", file, "}\n",sep = "")
cat("\\caption{SSA- (top) and MSE- (bottom) predictors for the first target series (left) and the second target series (right). Predictors applied to $\\epsilon_{1t}$ (blue) $\\epsilon_{2t}$ (red)", sep = "")
cat("\\label{filt_coef_var1}}", sep = "")
cat("\\end{center}")
cat("\\end{figure}")
@
<<label=ats_mba_2,echo=FALSE,results=tex>>=
xtable(perf_all, dec = 1,digits=rep(3,dim(mat_re)[2]+1),
...
\subsection{Application to a Multivariate Process with Heteroclite Targets}
...
gamma_target_1<-c(0.7^(1:L),c(rep(1/L_ma1,L_ma1),rep(0,L-L_ma1)),ARMAtoMA(ar=c(1.5,-.8),lag.max=L))
# Target z_{2t}
gamma_target_2<-c(c(rep(1/L_ma2,L_ma2),rep(0,L-L_ma2)),c(rep(1/L_ma3,L_ma3),rep(0,L-L_ma3)),ARMAtoMA(ar=c(0.5,-.9),lag.max=L))
# Target z_{3t}
gamma_target_3<-c(c(rep(1/L_ma3,L_ma3),rep(0,L-L_ma3)),0.9^(1:L),ARMAtoMA(ar=c(1.2,-.6),lag.max=L))
...
ht_vec<-matrix(c(min(4,L/2),min(6,L/2),min(10,L/2)),nrow=1)
rho0<-apply(ht_vec,1,compute_rho_from_ht)[[1]]$rho
with_negative_lambda<-T
# White noise input
xi<-NULL
...
#!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!  
lower_limit_nu<-"0"
# nu>2 we do not achieve the limit of admissibility but no unit-roots and unique solution
lower_limit_nu<-"2"
# nu>2*rhomax(L) i.e. we achieve the limit of admissibility: uniqueness and possibly unit-roots
# We use this setting which is the default
lower_limit_nu<-"rhomax"
...
# 2. Generate yt, zt and z_mse from eps_mat
  ...
  {
# Plot coefficients  
    mplot<-cbind(bk_mat[1:L,m],bk_mat[(L+1):(2*L),m],bk_mat[(2*L+1):(3*L),m])
    colo<-c("blue","red","green")
    ts.plot(mplot,col=colo)
...
# Target: zt    
    ...
    for (j in L:len)
      z[j]<-sum(apply(gammak*eps_mat[j:(j-L+1),],2,sum))
# Alternative target: MSE (this is used in SSA_func internally)   
    gammak_n<-c(c(gamma_target[m,2:L],0),c(gamma_target[m,L+2:L],0),c(gamma_target[m,2*L+2:L],0))
    z_mse<-rep(NA,len)
    for (j in L:len)
      z_mse[j]<-sum(apply(gammak_n*eps_mat[j:(j-L+1),],2,sum))
...
#------------
# Performances: empirical and theoretical criterion values and lag-one acfs
    perf_mat<-rbind(perf_mat,c(cor(na.exclude(cbind(z_mse[1:len],y[1:len])))[1,2], SSA_obj$crit_rhoyz[m],length(na.exclude(y))/length(which(y[(L+1):len]*y[L:(len-1)]<0)), ht_vec[m]))
# Criterion value is with respect to MSE-target: 
#   Optimized solutions are identical but criterion measures performances against MSE i.e. against 'benchmark'    
  } 
  colnames(perf_mat)<-c("Empirical criterion","Theoretical criterion","Empirical holding time","theoretical holding-time")
...
#   The criteria computed here correspond to the values in perf_mat_SSA above  
  ...
  save(file=paste(path.result,"bk_mat_heteroclite",sep=""),bk_mat)
  save(file=paste(path.result,"gammak_mse_heteroclite",sep=""),gammak_mse)
  save(file=paste(path.result,"perf_mat_heteroclite",sep=""),perf_mat)
} else
{
  load(paste(path.result,"bk_mat_heteroclite",sep=""))
  load(file=paste(path.result,"gammak_mse_heteroclite",sep=""))
  load(paste(path.result,"perf_mat_heteroclite",sep=""))
...
@
<<label=init,echo=FALSE,results=hide>>=
file = "filt_coef_heteroclite.pdf"
pdf(file = paste(path.out,file,sep=""), paper = "special", width = 6, height = 6)
...
cat("\\begin{center}")
cat("\\includegraphics[height=3in, width=6in]{", file, "}\n",sep = "")
cat("\\caption{SSA- (top) and MSE- (bottom) predictors for the first target series (left), second target (middle) and third target (right). Predictors applied to $\\epsilon_{1t}$ (blue) $\\epsilon_{2t}$ (red) and $\\epsilon_{3t}$ (green)", sep = "")
cat("\\label{filt_coef_heteroclite}}", sep = "")
cat("\\end{center}")
cat("\\end{figure}")
@
...
\subsection{Signal Extraction with Heteroclite Targets}
...
ts.plot(bk_x_mat[1:L,],col=colo)
ts.plot(bk_x_mat[L+1:L,],col=colo)
ts.plot(bk_x_mat[2*L+1:L,],col=colo)
...
# Check: both x are the same up to finite-length of Wold-decomposition   
...
# 2. use deconvoluted solution bk_x_mat applied to xt (obtained from theorem after inversion of convolution) 
    bk<-NULL
    for (j in 1:dim(Sigma)[1])
...
#------------
# Performances: empirical and theoretical criterion values and lag-one acfs
    perf_mat<-rbind(perf_mat,c(cor(na.exclude(cbind(z[1:len],y[1:len])))[1,2], SSA_obj$crit_rhoyz[m],length(na.exclude(y))/length(which(y[(L+1):len]*y[L:(len-1)]<0)), ht_vec[m]))
# Criterion value is with respect to MSE-target: 
#   Optimized solutions are identical but criterion measures performances against MSE i.e. against 'benchmark'    
  } 
  colnames(perf_mat)<-c("Empirical criterion","Theoretical criterion","Empirical holding time","theoretical holding-time")
...
cat("\\begin{center}")
cat("\\includegraphics[height=3in, width=6in]{", file, "}\n",sep = "")
cat("\\caption{SSA applied to $x_t$ (deconvolution: top graphs), SSA applied to $\\epsilon_t$ (convolution, middle graphs) and MSE- applied to $\\epsilon_t$ (bottom) predictors for the first target series (left), second target (middle) and third target (right). Predictors applied to $x_{1t}$ or $\\epsilon_{1t}$ (blue) $x_{2t}$ or $\\epsilon_{2t}$ (red) and $x_{3t}$ or $\\epsilon_{3t}$ (green)", sep = "")
