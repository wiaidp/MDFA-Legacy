...
A proof is provided by Kedem (1986). We can now formalize 'smoothness' of a predictor $y_t$ by constraining $\mathbf{b}$ such that
...
\subsection{Example 1: Forecasting a MA(2) Process}\label{one_step_fore}
...
\subsection{Example 2: Illustration of the Extension to Autocorrelated Processes}\label{example_autocor}
...
bk_mat=cbind(bk_mat,SSA_obj$bk_mat)
bk_white_noise_mat<-cbind(bk_white_noise_mat,SSA_obj$bk_white_noise_mat,xi,c(xi[2:length(xi)],0))
...
cat("\\begin{center}")
cat("\\includegraphics[height=3in, width=6in]{", file, "}\n",sep = "")
cat("\\caption{SSA arbitrarily scaled. All lags (left panel) and first ten lags (right panel). Filters as applied to ARMA-process (upper panels) vs. filters as applied to white noise of MA-inversion (bottom panels). The shaded lines in the bottom-right panel correspond to the SSA-filters in the previous MA(2)-example.", sep = "")
...
where $x_t=\Sexpr{ar1}x_{t-1}+\epsilon_t+\Sexpr{ma[1]}\epsilon_{t-1}+\Sexpr{ma[2]}\epsilon_{t-2}$ is a stationary and invertible ARMA-process with MA-inversion or Wold-decomposition $x_t=\sum_{k=0}\xi_k\epsilon_{t-k}$, displayed in figure \ref{filt_coef_example1_arma} (black lines in bottom left and right panels). The convolutions $(b\cdot\xi)_j$, $j=0,...,L-1$ proposed in section \ref{ext_stat}, obtained by applying SSA to $\epsilon_t$ of the MA-inversion of $x_t$ are displayed in the bottom panels, together with a comparison of the predictors in the previous example, where $z_t$ is a comparatively simple MA(2)-process, (bottom right panel). The 'ordinary' $\mathbf{b}$, as applied to $x_t$ instead of $\epsilon_t$, can be obtained by inversion of the convolutions $(b\cdot\xi)_j$
\[b_j=\left\{\begin{array}{cc}(b\cdot\xi)_0/\xi_{0}&,j=0\\
\frac{(b\cdot\xi)_{k+1}-\sum_{j=0}^{k}\xi_{k+1-j}b_j}{\xi_0}&j>0\end{array}\right.
\]
...
\subsection{Example 3: Smoothing and Un-Smoothing}\label{smooth_unsmooth}
...
xi<-c(1,ARMAtoMA(ar=ar1,ma=ma,lag.max=L_long-1))
ts.plot(xi)
ht_target<-compute_holding_time_func(xi)$ht
...
where $x_t=\Sexpr{ar1}x_{t-1}+\epsilon_t+\Sexpr{ma[1]}\epsilon_{t-1}+\Sexpr{ma[2]}\epsilon_{t-2}$ is a stationary and invertible ARMA-process. The target in this example is already 'smooth' with holding-time $ht=$\Sexpr{round(ht_target,1)}. We now apply two SA-designs with holding-times \Sexpr{round(ht_first,1)} and \Sexpr{round(ht_second,1)}: the first predictor 'un-smooths', the second smooths. Fig.\ref{filt_coef_example1_arma_su} displays filter coefficients as applied to $x_t$, left panel, or to $\epsilon_t$ in the Wold-decomposition of $x_t$, right panel. Un-smoothing is obtained by alternating signs of the (blue) predictor weights. SSA-predictors and target (shifted by $\delta=\Sexpr{delta}$) are compared in fig.\ref{output_example1_arma_su}: all series are arbitrarily scaled to unit-variance. Zero-crossings of the un-smoothing predictor are most frequent, followed by the target and the smoothing predictor.    




\subsection{Example 4: Convolution, Plancherel Identity and Amplitude Function}\label{conv_amp}


The difference-equation \ref{ar2} suggest that $\mathbf{b}$ can be interpreted as the convolution of the (non-stationary) AR(2)-filter with AR-coefficients $a_1=-\nu$, $a_2=1$ with the MSE-filter $\boldsymbol{\gamma}_{\delta}$. In fact, \ref{diff_non_home} and \ref{rho_fd} can be interpreted as convolutions and Plancherel-identity as stated in the frequency-domain. Fig.\ref{amp_shift_SSA} displays the amplitude functions of both SSA-predictors of the previous example when applied to $x_t$ (left panel) and $\epsilon_t$ from the MA-inversion (right panel). Both predictors are lowpass-designs when applied to $\epsilon_t$ (right) but the un-smoothing predictor as applied to $x_t$ (blue line left panel) is a bandpass design  emphasizing high-frequency-components.
<<label=init,echo=FALSE,results=hide>>=
K<-600
plot_T<-T
# SSA applied to ARMA
...
mplot<-scale(mat_amp_wn,center=F,scale=T)
...
cat("\\begin{center}")
cat("\\includegraphics[height=3in, width=6in]{", file, "}\n",sep = "")
cat("\\caption{Amplitude functions of SSA AR(2)-filters as applied to ARMA-process (left) and to white noise in MA-inversion (right).", sep = "")
...
\subsection{Example 5: Two Hyperparameters and the Smoothness-Timeliness Dilemma}\label{time_smooth}
...
\subsection{Example 6: Monotonicity vs. Non-Monotonicity of the Lag-one ACF}\label{mon_non_mono}
...
\subsection{Example 7: Unit-Root Case}
...
@
Typically, the coefficients of $\mathbf{b}$ decay towards zero at a rate determined by $\boldsymbol{\gamma}_{\delta}$ and the holding-time constraint $\rho_1$ or $ht_1$. Figure \ref{filt_coef_example4} displays two SSA-predictors with the same holding-time $\Sexpr{round(ht,1)}$ but with different filter-lengths $L=\Sexpr{L_long}$ (blue) and $L=\Sexpr{L_very_long}$ (red). The shorter filter is subject to a near unit-root because the holding-time is close to the maximal limit $\rho_max(50)$ for that filter length: its coefficients do not follow an exponential decay. In contrast, increasing the filter length leads to an exponentially decaying pattern at higher-order lags. In typical applications, the unit-root case typically occurs when $L$ and $ht_1$ are mismatched: in this case, filter performances are affected because the holding-time constraint 'overrules' the proper target-fitting. Increasing $L$ unleashes degrees of freedom and improves MSE-performances.   
<<label=init,echo=FALSE,results=hide>>=
...
\subsection{Example 8: Application to a Target with Incomplete Spectral Support}\label{incomplete_support}
...
\section{Application to BCA}\label{zcc-criter}\label{busi_cyc}
