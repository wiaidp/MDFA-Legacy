...
We propose a  generic forecast approach which merges various facets of the prediction problem in terms of mean-square error, sign-accuracy, zero-crossings and smoothness. The latter is formalized by a novel holding-time constraint which conditions the frequency of sign-changes by the predictor. Zero-crossings or sign-changes of a time series can be influential in the decision-making, for e.g. economic actors, by marking transitions between up- and down-turns, expansions and recessions, bull and bear markets, and our forecast approach contributes to such a design of the predictor. A solution to this problem has been derived in the frequency-domain in the case of univariate prediction problems. We here propose a time-domain approach with extensions to multivariate forecast problems. Also, closed-form solutions of the non-linear problem are obtained for some special cases.     
...
Time series forecasting aims at a coherent analysis of the main systematic dynamics of a phenomenon in view of synthesizing information about future events. Typically, the forecast process is structured by a formal optimality concept, whereby a particular forecast-error measure, such as e.g. the mean-square error (MSE), is minimized. We here argue that multiple and various characteristics of a predictor might draw attention such as the smoothing capability, i.e. the extent by which undesirable 'noisy' components of a time series are suppressed, or timeliness, as measured by relative lead or lag properties of a predictor, or sign accuracy and zero-crossings, as measured by the ability to predict the correct sign of a target. Wildi (2023 b) proposes a generic forecast approach, referred to as simple sign-accuracy (SSA), by merging sign-accuracy and MSE performances subject to a holding-time constraint which  determines the expected number of zero-crossings of the predictor in a time interval of given length. Zero-crossings (of the growth-rate) of a time series are influential in the decision-making, for e.g. economic actors, by marking transitions between up- and down-turns, expansions and recessions, bull and bear markets, and our forecast approach contributes to such a design of the predictor. Wildi (2023 b) derives a solution to the univariate forecast problem as stated in the frequency-domain. We here provide an alternative time-domain perspective and extend the solution to the case of multivariate prediction problems. Interestingly, the structure of the problem as entailed in the time-domain allows for a derivation of a closed form solution of the non-linear problem in terms of roots of palyndromic polynomials some special cases. 
% in terms of an interpretable hyper-parameter. % While a comprehensive and formal treatment of timeliness must be deferred, corresponding issues will be considered indirectly, via an additional and interpretable tuning- or hyper-parameter, and performances in terms of leads or lags will be measured accordingly. %Some of our examples illustrate that classic predictors can be outperformed both in terms of smoothness and timeliness at once.       
%We here combine mean-square error (MSE) performances, sign accuracy, zero-crossings and smoothness characteristics in a common formal framework under suitable assumptions about the data-generating process. % and defer a lengthier theoretical treatment of timeliness which will be considered from a purely descriptive perspective, only. 
McElroy and Wildi (2019) propose an alternative methodological framework for addressing specific facets of the forecast problem but their approach does not account for zero-crossings explicitly which may be viewed as a shortcoming in some applications. Wildi (2023 a) proposes an application of the univariate SSA to a (real-time) business-cycle analysis, but the chosen treatment remains  largely informal.  \\
...
The optimization criterion is derived in section \ref{zc} with a discussion of robustness and extensions of the basic methodological framework (BMF); solutions of the criterion are proposed in section \ref{theorem_SSA} with a discussion of boundary and singular cases, numerical aspects as well as the sample distribution of the estimate; sections \ref{examples} and \ref{sign_het} propose applications to  forecasting and (real-time) signal extraction. All empirical examples are reproducible in an open source R-package (include link to Github). Finally, section \ref{conclusion} concludes by summarizing our main findings. 
...
Let $\mathbf{x}_t=(x_{1t},...,x_{nt})'$ be a multivariate stationary process with (purely non-deterministic) Wold-decomposition $\mathbf{x}_t=\sum_{k=0}^\infty \boldsymbol{\Xi}_k\boldsymbol{\epsilon}_{t-k}$ where $\boldsymbol{\Xi}_0=\mathbf{I}_{n*n}$ is the identity and where $\boldsymbol{\epsilon}_{t}=(\epsilon_{1t},...,\epsilon_{nt})'$ is a sequence of multivariate Gaussian noise with variance-covariance matrix $\boldsymbol{\Sigma}$: denote by $\tilde{\sigma}_{j}>0$, $j=1,...n$, the eigenvalues of $\boldsymbol{\Sigma}$ (full rank)\footnote{If $\boldsymbol{\Xi}_0\neq \mathbf{I}_{n*n}$ then one can multiply the system with $\boldsymbol{\Xi}_0^{-1}$, which is assumed to exist because $\boldsymbol{\Sigma}>0$, and obtain a transformed variance covariance matrix $\boldsymbol{\Xi}_0^{-1}\boldsymbol{\Sigma}\boldsymbol{\Xi}_0$.}. Let $\mathbf{z}_t=\sum_{|k|<\infty}\boldsymbol{\Gamma}_k\mathbf{x}_{t-k}$ be a stationary Gaussian series, where $\boldsymbol{\Gamma}_k$, with entries $(\gamma_{ijk})$ $i,j=1,...,n$, denotes a sequence of square-summable filter matrices i.e. $\sum_{|k|<\infty}\boldsymbol{\Gamma}_k'\boldsymbol{\Gamma}_k<\infty$ elementwise. We consider estimation of $\mathbf{z}_{t+\delta}$, where $\delta\in \mathbb{Z}$, based on the one-sided predictor $\mathbf{y}_t=\sum_{k=0}^{L-1}\mathbf{B}_k\mathbf{x}_{t-k}$.  Let us denote
\begin{eqnarray*}
\mathbf{x}_{it}&=&(x_{it},x_{it-1},...,x_{it-(L-1)})'\\
\mathbf{x}_{ t}&=&(\mathbf{x}_{1t}',...,\mathbf{x}_{nt}')'\\
\mathbf{B}_k&=&\left(\begin{array}{ccc}b_{11k}&...&b_{1nk}\\&...&\\b_{n1k}&...&b_{nnk}\end{array}\right)\\
...
{\mathbf{y}}_t&=&\mathbf{b}\mathbf{x}_{t}\\
{{y}}_{it}&=&\mathbf{b}_i\mathbf{x}_{t}
\end{eqnarray*}
In order to introduce the relevant topics we first assume $\mathbf{x}_t=\boldsymbol{\epsilon}_t$ so that   $\boldsymbol{\Xi}_k=\mathbf{0}$ for $k>0$. We refer to this simplifying setting in terms of basic methodological framework (BMF). The BFM chiefly intends to clarify exposition and to simplify notation in view of highlighting the relevant facets of the prediction problem in a decluttered formal context. Extensions to  autocorrelated $\mathbf{x}_t$ are discussed in section \ref{ext_stat} and departures from the Gaussian assumption are addressed in section \ref{non_gaussian}. Let then
\begin{eqnarray*}
...
\end{eqnarray*}   
The (classic) MSE-predictor $\hat{\mathbf{z}}_{t\delta}$ of $\mathbf{z}_{t+\delta}$ is obtained by substituting zeroes for those $\epsilon_{jk}$, $j=1,...,n$, in $\mathbf{z}_{t+\delta}$ for which  $k\notin\{0,...,L-1\}$ (orthogonal projection): it can be compactly expressed as
\begin{eqnarray*}
\hat{\mathbf{z}}_{t\delta}&=&\boldsymbol{\gamma}_{\delta}\boldsymbol{\epsilon}_{t}
\end{eqnarray*}
Let also  
\[
M=\left(\begin{array}{ccccccccc}0&0.5&0&0&0&...&0&0&0\\
...
\end{array}\right)
\]
of dimension $L*L$ designate the so-called autocovariance-generating matrix, with eigenvalues $\lambda_k=-\cos(k\pi/(L+1))$, k=1,...L, ordered according to increasing size. Then 
\begin{eqnarray}
E[z_{it+\delta}y_{it}]&=&\boldsymbol{\gamma}_{i\cdot\delta}'\tilde{\mathbf{I}}\mathbf{b}_{i}\label{moment1}\\
E[z_{it+\delta}^2]&=&\boldsymbol{\gamma}_{i\cdot\delta}'\tilde{\mathbf{I}}\boldsymbol{\gamma}_{i\cdot\delta}\label{moment2}\\
...
\end{eqnarray}
where $\tilde{\mathbf{I}}=\boldsymbol{\Sigma}\otimes\mathbf{I}_{L*L}$ and $\tilde{\mathbf{M}}=\boldsymbol{\Sigma}\otimes\mathbf{M}$ are the $nL*nL$-dimensional Kronecker products of the $n*n$ dimensional variance $\boldsymbol{\Sigma}$ and the $L*L$ dimensional identity  $\mathbf{I}_{L*L}$ and $\mathbf{M}$, respectively. Finally, the variance of $z_{it}$ can be expressed as
\begin{eqnarray}\label{moment5}
E[z_{it}^2]=\sum_{|k|\infty}\boldsymbol{\gamma}_{i k}'\boldsymbol{\Sigma}\boldsymbol{\gamma}_{i k}
\end{eqnarray}
where $\boldsymbol{\gamma}_{i k}:=(\gamma_{i1k},...,\gamma_{ink})'$. 

\subsection{Sign-Accuracy, MSE and Holding-Time}\label{mse_sa_zc}

For each $i=1,...,n$ we  look for an estimate $y_{it}=\mathbf{b}_i'\mathbf{x}_t$ of $z_{it+\delta}$ such that the probability P$\Big(\sign(z_{it+\delta})=\sign(y_{it})\Big)$ is maximized as a function of $\mathbf{b}_i$ and we refer to this criterion in terms of \emph{sign accuracy} (SA). 
...
\end{eqnarray*}
are the correlations between $y_{it}$ and the target $z_{t+\delta}$ and the MSE-estimate $\hat{{z}}_{it\delta}$ of $z_{it+\delta}$. 
\end{Proposition}

Proof\\
In the stipulated case of Gaussian random variables 
\begin{eqnarray}
P\Big(\sign(z_{it+\delta})=\sign(y_{it})\Big)&=&0.5+\frac{\arcsin(\rho(y_i,z_i,\delta))}{\pi}\label{arcsin}\\
&=&0.5+\frac{\arcsin(\rho(y_i,\hat{{z}}_{it\delta},\delta))}{\pi}\nonumber
\end{eqnarray}
The first equality follows from the Gaussian assumption and the second equality is obtained from orthogonal projection of $z_{it+\delta}$ onto the space spanned by $\epsilon_{jt-k}$, $j=1,...,n$, $0\leq k\leq L-1$ ($y_{it}$ does not depend on $\epsilon_{jt-k}$ for $k\notin\{0,...,L-1\}$). The proof then follows from \ref{moment1},\ref{moment2}, \ref{moment3}, \ref{moment5} and from strict monotonicity of the non-linear arcsin-transformation. \\
...
@
% (zero-crossings or correlations are insensitive to arbitrary scalings.\\ %However, we maintain the above formulation which will prove insightful when generalizing the optimization concept.  \\
Consider now the expected duration between consecutive zero-crossings or sign-changes of the predictor $y_{it}$, which will be referred to as \emph{holding-time}.

\begin{Proposition}\label{ht_formula}
Under the BMF the holding-time $ht(y_i|\mathbf{b})$ of $y_{it}$ is 
\begin{eqnarray}\label{ht}
ht(y_i|\mathbf{b}_i)=\frac{\pi}{\arccos(\rho(y_i,y_i,1))}
\end{eqnarray}
where $\rho(y_i,y_i,1)=\displaystyle{\frac{\mathbf{b}_i'\mathbf{\tilde{M}}\mathbf{b}_i}{\mathbf{b}_{i}'\tilde{\mathbf{I}}\mathbf{b}_{i}}}$ is the lag-one autocorrelation (acf) of $y_{it}$. 
\end{Proposition}

A proof is provided by Kedem (1986), relying on \ref{moment3} and \ref{moment4} for the lag-one acf. We can now formalize the concept of 'smoothness' of a predictor $y_{it}$ by constraining $\mathbf{b}_i$ such that
\begin{equation}\label{ht_const}
...
\rho(y_i,y_i,1)= \rho_i
\end{equation}
where $ht_i$ or $\rho_i$, linked through \ref{ht}, are proper hyper-parameters of our design. In the following, we  refer to the 'holding-time' either in terms of $ht(y_i|\mathbf{b}_i)$ or $\rho(y_i,y_i,1)$, clarifying our intent in case of ambiguity. If $ht_i$ differs from the holding-time of the MSE-predictor $\hat{{z}}_{i\delta}$ then under suitable regularity assumptions (see theorem \ref{lambda_mult}) $y_{it}\not\propto \hat{{z}}_{i\delta}$ where $\not \propto$ means 'not proportional to' (so-called non-degenerate case, see theorem \ref{lambda_mult}). We here argue that the hyper-parameter $ht_i$ is interpretable and can be set a priori, at the onset of an analysis, according to structural elements of a prediction problem. As an example, Wildi (2023 a) illustrates the proceeding in a (univariate) business-cycle application, where $ht_1$ matches the length of historical recession episodes. Also, the holding-time could be selected in view of taming the number of unsystematic or noisy crossings (false alarms). Furthermore, if costly strategy-adjustments or behavioral changes take place at zero-crossings, for example in an algorithmic trading framework or for macro-economic policy setting, then cumulated costs of all adjustments, being inversely proportional to $ht_i$, could be  accounted for by a suitable determination of the hyperparameter. Finally, $ht_i$ could be set according to short-, mid- or long-term i.e tactic, strategic or fundamental outlook perspectives. Concerning the proper selection of the holding-time, the following proposition sets limits for admissible constraints in the basic framework.

\begin{Proposition}\label{maxrho}
Under the BMF, maximal and minimal lag-one autocorrelations $\rho_{max}(L),\rho_{min}(L)$ of $y_{it}$ are $ \rho_{max}(L)=-\rho_{min}(L)=\cos(\pi/(L+1))$. The corresponding MA-coefficients $b_{max,k}:=\sin\left(\displaystyle{\frac{(1+k)\pi}{L+1}}\right)$, $k=0,...,L-1$, and $b_{min,k}:=(-1)^kb_{max,k}$ do not depend on $i$ and are are uniquely determined down to arbitrary scaling and sign.  
\end{Proposition}

Proof\\
Re-group $y_{it}=\mathbf{b}_i'\boldsymbol{\epsilon}_t$ as in $y_{it}=\sum_{k=0}^{L-1}\left(\sum_{j=1}^nb_{ijk}\epsilon_{jt-k}\right)=\sum_{k=0}^{L-1}\tilde{b}_k\nu_{t-k}$ where $\nu_t$ is iid Gaussian noise. Then the proof of the proposition follows from N. Davies, M. B. Pate and M. G. Frost (1974), see also proposition \ref{stationary_eigenvec} further down.\\ 
%is a hyper-parameter that controls for the \emph{smoothing}-capability of the filter $\mathbf{b}$. %y_t$ by imposing a mean-length between consecutive zero-crossings. 
...
\right\}
\end{eqnarray}
This optimization problem is called \emph{simple sign-accuracy} or SSA-criterion: simplicity here refers to the elementary structure of the predictor, as derived in theorem \ref{lambda}. We allude  to solutions of this criterion by the acronym SSA or SSA($ht_i,\delta$) or SSA($\rho_i,\delta$) to stress the dependence of the predictor on the pair of hyper-parameters, see section \ref{time_smooth} for reference. The SSA-criterion merges MSE, sign accuracy and smoothing requirements in a flexible and consistent way. Departures from the Gaussian assumption can be accommodated in the sense that $y_{it}$ or $z_{it}$ can be 'nearly Gaussian' even if $x_{it}=\epsilon_{it}$ is not, due to the central limit theorem, see Wildi (2023 a) for an application to economic data, Wildi (2023 b) for an application to heavy-tailed data (both in a univariate framework) and section \ref{non_gaussian} in the multivariate case. Finally, the  criterion remains appealing outside of a strict holding-time or zero-crossing perspective by complementing the classic predictor with a generic smoothing constraint addressing the lag-one acf.        
<<label=init,results=hide>>=
# Purposes
# 0. Use Gauss or student-t (if skewed then one has to shift by mean)
# 1. Check that MSE/correlation has same sign-accuracy as logit, in-sample
...
\end{eqnarray*} 
where $(\boldsymbol{\Gamma}\cdot\boldsymbol{\Xi})_k =\sum_{m\leq k}\boldsymbol{\Gamma}_m\boldsymbol{\Xi}_{k-m}$ and $(\mathbf{B}\cdot\boldsymbol{\Xi})_j=\sum_{n=0}^{\min(L-1,j)}\mathbf{B}_n \boldsymbol{\Xi}_{j-n} $ are convolutions of the sequences $\boldsymbol{\Gamma}_k$ and $\mathbf{B}_j$ with the Wold-decomposition $\boldsymbol{\Xi}_m$ of $\mathbf{x}_t$. For given, $(\mathbf{B}\cdot\boldsymbol{\Xi})_j$, the 'original' $\mathbf{B}_{k}$ can be obtained from deconvolution
\begin{eqnarray}\label{con_inv}
\mathbf{B}_{k}&=&\left\{\begin{array}{cc}(\mathbf{B}\cdot\boldsymbol{\Xi})_{0}\boldsymbol{\Xi}_0^{-1}&,k=0\\
\left(  (\mathbf{B}\cdot\boldsymbol{\Xi})_{k}-\sum_{n=0}^{k-1}\mathbf{B}_n \boldsymbol{\Xi}_{j-n}\right)\boldsymbol{\Xi}_0^{-1}&,k=1,...,L-1
\end{array}\right.
\end{eqnarray}
This expression could be simplified by noting that $\boldsymbol{\Xi}_0^{-1}=\mathbf{I}$ in our parametrization. Let $(\boldsymbol{\Gamma}\cdot\boldsymbol{\Xi})_{klm}$ and $(\mathbf{B}\cdot\boldsymbol{\Xi})_{jlm}$ denote the $lm$ element of the corresponding matrices, assume that $\delta\geq 0$ (for- and nowcasting\footnote{Similar but notationally more cumbersome expressions would be obtained for $\delta<0$ (backcast) which are skipped here.}) and denote
\begin{eqnarray*}
(\boldsymbol{\gamma}\cdot\boldsymbol{\xi})_{ij\delta}&=&\left((\boldsymbol{\Gamma}\cdot\boldsymbol{\Xi})_{\delta ij},(\boldsymbol{\Gamma}\cdot\boldsymbol{\Xi})_{\delta+1 ij}...,(\boldsymbol{\Gamma}\cdot\boldsymbol{\Xi})_{\delta+L-1 ij}\right)'\\
...
(\mathbf{b}\cdot\boldsymbol{\xi})_{ij}&=&\left((\mathbf{B}\cdot\boldsymbol{\Xi})_{\delta ij},(\mathbf{B}\cdot\boldsymbol{\Xi})_{\delta+1ij},...,(\mathbf{B}\cdot\boldsymbol{\Xi})_{\delta+L-1ij}\right)'\\
...
\end{eqnarray}
which can be solved for $(\mathbf{b}\cdot\boldsymbol{\xi})_i$, see theorem \ref{lambda_mult}. $\mathbf{b}_i$ could then be obtained from deconvolution \ref{con_inv}. While $(\mathbf{b}\cdot\boldsymbol{\xi})_i$ is applied to $\boldsymbol{\epsilon}_t$, $\mathbf{b}_i$ can be applied to the original data $\mathbf{x}_t$: both predictors are identical up to negligible finite sample issues (the infinite-length Wold-decomposition of $\mathbf{x}_t$  is truncated at lag $k=L-1$). We refer to textbooks for a derivation of the multivariate Wold decomposition based on a finite sample $x_1,...,x_T$, see e.g. Brockwell and Davis (1993): a worked-out example is provided in section \ref{sign_het}.  Finally, for notational convenience we henceforth rely on the BMF, acknowledging that straightforward modifications would apply in the case of autocorrelation.     
  %From an empirical perspective, we argue that growth-rates of a wide range of economic time series are in accordance with our simplifying assumption, see e.g. the so-called 'typical spectral shape' of an economic variable in Granger (1966). %To conclude, we note that the procedure could be extended to non-stationary integrated processes. % and its utility would be questionable in the context of suitably transformed  data, typically differences or log-returns, at least if the transformation does not impede the analysis.   % assumption in terms of  conditional heteroscedasticity (vola-clustering) or so-called 'fat tails' (large kurtosis, outliers) is analyzed in section \ref{robustness_SSA}.
...
The following proposition relates stationary points of the lag-one acf $\rho(y_i,y_i,1)$ with eigenvectors and eigenvalues of   $\mathbf{\tilde{M}}$. 


\begin{Proposition}\label{stationary_eigenvec}
Under the BMF, $\mathbf{b}_i\neq 0$ is a stationary point of the lag-one autocorrelation $\rho(y_i,y_i,1)=\displaystyle{\frac{\mathbf{b}_i'\mathbf{\tilde{M}}\mathbf{b}_i}{\mathbf{b}_{i}'\tilde{\mathbf{I}}\mathbf{b}_{i}}}$ if and only if $\mathbf{b}_{ij}$, $j=1,...,m$, are an eigenvector of $\mathbf{{M}}$, the same for all $j$, with corresponding eigenvalue $\lambda_k$ such that $\lambda_k=\rho(y_{i},y_{i},1)=\rho(y_{ij},y_{ij},1)$ for some $k=1,...,L$. The extremal values $\rho_{min}(L)$ and $\rho_{max}(L)$ defined in proposition \ref{maxrho} correspond to $\min_i\lambda_i$ and $\max_i\lambda_i$. 
\end{Proposition}

Proof\\

Assume, for simplicity, that $\mathbf{b}_i\neq\mathbf{0}$ is defined on the 'unit-ellipse'  $\mathbf{b}_{i}'\tilde{\mathbf{I}}\mathbf{b}_{i}=1$ so that  
\begin{eqnarray*}
...
\]
Under the BMF $\boldsymbol{\Sigma}$ has full-rank so that we can multiply with $\boldsymbol{\Sigma}^{-1}\otimes\mathbf{I}_{L*L}=\mathbf{\tilde{I}}^{-1}$ to obtain
<<label=init,echo=FALSE,results=hide>>=
# Check claim 
n<-3
L<-5
...
\begin{eqnarray*}
y_{it}=\mathbf{b}_i'\boldsymbol{\epsilon}_t\propto\mathbf{v}_{k}'\left(\sum_{j=1}^n\boldsymbol{\epsilon}_{jt}\right)
\end{eqnarray*} 
so that $\rho(y_i,y_i,\delta)=\rho(y_{ij},y_{ij},\delta)=\lambda_k$, as claimed. Finally, since the  unit-ellipse is free of boundary-points, we conclude that the extremal values $\rho_{min}(L)$, $\rho_{max}(L)$ of the lag-one acf of $y_{it}$ must be stationary points i.e. $\rho_{min}(L)=\min_i\lambda_i$ and $\rho_{max}(L)=\max_i\lambda_i$ which completes the proof of the proposition.\\
<<label=init,echo=FALSE,results=hide>>=
# Check cosine formula for eigenvalues, see e.g. Anderson
L<-11
M<-matrix(nrow=L,ncol=L)
...
Designate by $\tilde{\lambda}_m$, $m=1,...,nL$ the eigenvalues of $\mathbf{\tilde{M}}$ with corresponding eigenvector $\mathbf{\tilde{v}}_m$. Note that $\tilde{\lambda}_m=\lambda_k\tilde{\sigma}_j$ for some combination of eigenvalues $\lambda_k$ and $\tilde{\sigma}_j$: we then denote eigenvalues and eigenvectors of $\tilde{\mathbf{M}}$ alternatively by $\tilde{\lambda}_{kj}=\lambda_k\tilde{\sigma}_j$ and $\mathbf{\tilde{v}}_{kj}$. The eigenvalues of $\mathbf{\tilde{I}}$ are $\tilde{\sigma}_j$ with multiplicity $L$ for each $j=1,...,n$ and the eigenvectors $\mathbf{\tilde{v}}_{kj}$ of $\mathbf{\tilde{M}}$ are also eigenvectors of $\mathbf{\tilde{I}}$. Grouping the orthonormal eigenvectors $\mathbf{\tilde{v}}_{kj}$ in the $nL*nL$-dimensional matrix $\mathbf{\tilde{V}}$ we can then express the MSE-filter $\boldsymbol{\gamma}_{i\cdot\delta}$ by the spectral factorization  
\begin{eqnarray}\label{specdec_mult}
\boldsymbol{\gamma}_{i\cdot\delta}=\mathbf{\tilde{V}}\mathbf{w}_i
\end{eqnarray}
where $\mathbf{w}_i=w_{ikj}, k=1,...,L, j=1,...,n$ designate the spectral weights of $\boldsymbol{\gamma}_{i\cdot\delta}$. In the sequel we will often refer to $\boldsymbol{\gamma}_{i\cdot\delta}$ as the 'target' in the sense that $y_{ti}$ should fit $\hat{z}_{it\delta}$,  recall proposition \ref{sa_crit_mult}, or, equivalently, $\mathbf{b}_i$ should match $\boldsymbol{\gamma}_{i\cdot\delta}$ conditional on the holding-time constraint.  
The target $\boldsymbol{\gamma}_{i\cdot\delta}$ is called \emph{band-limited} if $w_{i1j}=0$, for $j=1,...,n$, or if $w_{iLj}=0$ for $j=1,...,n$. Recall that our indexing $ikj$ refers to the ordering of the eigenvalues $\tilde{\lambda}_{kj}=\lambda_k\tilde{\sigma}_j$ of $\mathbf{\tilde{M}}$ whereby $\lambda_k$ are aligned according to increasing size: therefore $w_{i1j}$ and $w_{iLj}$ refer to the eigenvectors $\mathbf{\tilde{v}}_{1j}$ and $\mathbf{\tilde{v}}_{Lj}$ corresponding to $\lambda_1\tilde{\sigma}_j$ and $\lambda_L\tilde{\sigma}_j$. Finally, $\boldsymbol{\gamma}_{i\cdot\delta}$ is said to have complete (or incomplete) spectral support depending on $\sum_{j=0}^n|w_{ikj}|\neq 0$ for all $k\in\{1,...,L\}$ (or not). Spectral completeness implies that $\boldsymbol{\gamma}_{i\cdot\delta}$ is not {band-limited} (but the converse is not true).  


\begin{Theorem}\label{lambda_mult}
Consider the SSA optimization problem \ref{crit1_mult} under the BMF and consider the following set of regularity assumptions for $i=1,...,n$:
\begin{enumerate}
...
\[
\mathbf{b}_i\propto\sum_{j=1}^n \tilde{\sigma}_jw_{i1j}\tilde{\mathbf{v}}_{1j}
\]
where $w_{ijk}$ and $\mathbf{\tilde{v}}_{1j}$ are the spectral weights and vectors in \ref{specdec_mult}. If $w_{i1j}=0$ for $j=1,..n$ then the last regularity assumption (completeness) is violated too and the problem is ill-conditioned. If $\rho_i=\lambda_L=\rho_{max}(L)$  then similarly
\[
\mathbf{b}_i\propto\sum_{j=1}^n \tilde{\sigma}_jw_{iLj}\tilde{\mathbf{v}}_{Lj}
\]
...
\mathbf{b}_{i}=D_i\boldsymbol{\nu}_i^{-1}\mathbf{\tilde{I}}\boldsymbol{\gamma}_{i\cdot\delta}=D_i\sum_{k=1}^L\frac{1}{(2\lambda_{k}-\nu_i)}\tilde{w}_{ik}\mathbf{\tilde{\tilde{v}}}_{ik}
\end{eqnarray}
where $D_i\neq 0$, $\nu_i\in \mathbb{R}-\{2\lambda_k|k=1,...,L\}$, $\boldsymbol{\nu}_i:=2\mathbf{\tilde{M}}-\nu_i\mathbf{\tilde{I}}$ is an invertible $nL*nL$ matrix, $\tilde{w}_{ik}=\sqrt{\sum_{j=1}^n w_{ikj}^2}$ and $\mathbf{\tilde{\tilde{v}}}_{ik}=\sum_{j=1}^n w_{ikj}\mathbf{\tilde{v}}_{kj}/|\tilde{w}_{ik}|$ is an orthonormal basis of an $L$-dimensional subspace $\tilde{\mathbb{R}}_i^{L}$ of $\mathbb{R}^{nL}$. Although $b_{ij,-1}$ and $b_{ijL}$, for $j=1,...,n$, do not explicitly appear in $\mathbf{b}_{i}$ it is at least implicitly assumed that $b_{ij,-1}=b_{ijL}=0$ (implicit boundary constraints). Furthermore, $\mathbf{b}_i$ is uniquely determined by the scalar $\nu_i$, down to the arbitrary scaling term $D_i$, whereby the sign of $D_i$ is such that $\boldsymbol{\gamma}_{i\cdot\delta}'\tilde{\mathbf{I}}\mathbf{b}_{i}>0$ (positive criterion value).
\item The lag-one acf $\rho_i(\nu)$ of $\mathbf{b}_{i}=\mathbf{b}_{i}(\nu_i)$ in \ref{ssa_mult} is
...
\end{eqnarray}
Any $\rho_i$ such that $|\rho_i|<\rho_{max}(L)$ is admissible in the holding-time constraint \ref{crit1_mult}.
...
@
The multivariate SSA-problem \ref{crit1_mult} can be rewritten for each $i=1,..,n$ as
\begin{eqnarray}
...
\end{eqnarray}
of $\mathbf{b}_i$ where the indexing corresponds to the ordering of the eigenvectors according to the eigenvalues $\tilde{\lambda}_{kj}=\lambda_k\tilde{\sigma}_j$ of $\mathbf{\tilde{M}}$, from smallest to largest $\lambda_i$. Note that $\tilde{\mathbf{v}}_{kj}$ is an eigenvector of $\mathbf{\tilde{I}}$ with eigenvalue $\tilde{\sigma}_j>0$ (the eigenvalues of $\boldsymbol{\Sigma}$). Therefore, the length-constraint $\mathbf{b}_i'\mathbf{\tilde{I}}\mathbf{b}_i=1$ implies 
...
\end{eqnarray*}
(hyperbolic\footnote{The eigenvalues $\lambda_k=-cos(k\pi/(L+1))$ are distributed symmetrically about zero.} holding-time constraint). We then select $k=2,j=1$ and solve the hyperbolic constraint for $\alpha_{21}$
...
\begin{eqnarray*}
\alpha_{11}^2\left(1-\frac{\lambda_1}{\lambda_2}\right)&=&\frac{1}{\tilde{\sigma}_1}\left(1-\frac{\rho_i}{\lambda_2}\right)+\sum_{(k,j)\notin\{(2,1),(1,1)\}}\left(\frac{\lambda_k\tilde{\sigma}_j}{\lambda_2\tilde{\sigma}_1}-\frac{\tilde{\sigma_j}}{\tilde{\sigma}_1}\right)\alpha_{kj}^2\\
&=&\frac{1}{\tilde{\sigma}_1}\left(1-\frac{\rho_i}{\lambda_2}\right)+\sum_{(k,j)\notin\{(2,1),(1,1)\}}\left(\frac{\lambda_k\tilde{\sigma}_j}{\lambda_2\tilde{\sigma}_1}-\frac{\tilde{\sigma_j}}{\tilde{\sigma}_1}\right)\alpha_{kj}^2
\end{eqnarray*}
Assume now that $\rho_i=\lambda_1$ and simplify the previous expression to obtain
\begin{eqnarray}
...
\end{eqnarray}
where $\sum_{(k,j)\notin\{(2,1),(1,1:n)\}}$ means the sum over all combinations excluding $k=1$ and $k=2,j=1$. We can rewrite this expression as
...
\end{eqnarray}
Since $\tilde{\sigma}_j\frac{\lambda_2-\lambda_k}{\lambda_2-\lambda_1}<0$ for all $k\neq 1$, we infer that the summands on the right must vanish: $\alpha_{kj}=0$ for all $k,j$ such that $(k,j)\notin\{(2,1),(1,1:n)\}$. Otherwise  $\sum_{j=1}^n \tilde{\sigma}_j\alpha_{1j}^2>1$ would contradict the elliptical length constraint \ref{length_const}. Then 
\begin{eqnarray}\label{reduced_rank_el_eq}
\sum_{j=1}^n \tilde{\sigma}_j\alpha_{1j}^2=1
\end{eqnarray} 
so that \ref{ito_v} implies $\alpha_{21}=0$. We deduce that if $\rho_i=\lambda_1$, then ellipse and hyperbole have a reduced-rank $(n-1)$-dimensional elliptical intersection as specified by \ref{reduced_rank_el_eq}. %\footnote{This solution generalizes the univariate case in Wildi (2023 b) where the intersection corresponds to the vertices $\pm \mathbf{v}_1$ i.e. the eigenvector of $\mathbf{M}$ belonging to the smallest eigenvalue $\lambda_1$.}. Note that since $\mathbf{\tilde{v}}_{1j}$???  
The solution of the SSA-problem becomes
\[
\mathbf{b}_i=\sum_{j=1}^n \tilde{\sigma}_j\alpha_{1j}\tilde{\mathbf{v}}_{1j}
\]
We now consider the criterion \ref{crit_ssa_proof} 
\[
\boldsymbol{\gamma}_{i\cdot\delta}'\tilde{\mathbf{I}}\mathbf{b}_{i}=\sum_{j=1}^n w_{i1j}\tilde{\sigma}_j^2\alpha_{1j}
\]
As a result, the SSA-problem becomes
\begin{eqnarray*}
\max_{\alpha_{1j}}\sum_{j=1}^n w_{i1j}\tilde{\sigma}_j^2\alpha_{1j}~~\textrm{subject~to}~~\sum_{j=1}^n \tilde{\sigma}_j\alpha_{1j}^2=1
\end{eqnarray*}
with adjoined Lagrange-equations $\tilde{\sigma}_j^2w_{i1j}-2\lambda\tilde{\sigma}_j\alpha_{1j}=0$ so that
\[
\mathbf{b}_i\propto\sum_{j=1}^n \tilde{\sigma}_jw_{i1j}\tilde{\mathbf{v}}_{1j}
\]
...
\end{eqnarray*}
Since the ellipse $\mathbf{b}_{i}\mathbf{\tilde{I}}\mathbf{b}_{i\cdot}=1$ is free of boundary points, the solution $\mathbf{b}_{i}$ of the SSA-problem must conform to the stationary Lagrangian or vanishing gradient equations
\[
...
\]
Note that the second regularity assumption of theorem \ref{lambda} (non-degenerate case) implies that the holding-time constraint \ref{hyperbola_mult} is 'active' so that $\lambda_{i2}\neq 0$.  Dividing by $\lambda_{i2}$ then leads to 
\begin{eqnarray}
D_i\tilde{\mathbf{I}}\boldsymbol{\gamma}_{i\cdot\delta}&=& \boldsymbol{\nu}_i\mathbf{b}_i\label{diff_non_hom_matrix_mult}\\
\boldsymbol{\nu}_i&:=&(2\mathbf{\tilde{M}}-\nu_i\mathbf{\tilde{I}})\label{labelNu_mult}
\end{eqnarray}
where $D_i=1/\lambda_{i2}$  and $\nu_i=-2\frac{\lambda_{i1}}{\lambda_{i2}}$. Since the SSA-solution  is realized on the $nL-2$-dimensional intersection of ellipse \ref{unit_circle_mult} and  holding-time hyperbola \ref{hyperbola_mult} (so-called full-rank case, see the proof of the first assertion), we infer that the  objective function is not overruled by the constraint so that $|\lambda_{i2}|<\infty$ and therefore $D_i\neq 0$ in \ref{diff_non_hom_matrix_mult}, as claimed. Furthermore, equation \ref{diff_non_hom_matrix_mult} can be written out explicitly 
\begin{eqnarray}\label{ar2_mult}
\sum_{m=1}^n\sigma_{jm}(b_{imk+1}-\nu_i b_{imk}+b_{imk-1})&=&D_i\sum_{m=1}^n\sigma_{jm}\gamma_{imk+\delta}~,~1\leq k\leq L-2\\
\sum_{m=1}^n\sigma_{jm}(b_{im1}-\nu_i b_{im0})&=&D_i\sum_{m=1}^n\sigma_{jm}\gamma_{im\delta}~,~k=0\nonumber\\
\sum_{m=1}^n\sigma_{jm}(-\nu_i b_{imL-1}+b_{imL-2})&=&D_i\sum_{m=1}^n\sigma_{jm}\gamma_{imL-1+\delta}~,~k=L-1\nonumber
\end{eqnarray}
for each $j=1,...,n$. A comparison of the last two equations with the first one leads to the implicit boundary constraints $\sum_{m=1}^n\sigma_{jm}b_{im,-1}=0$ and $\sum_{m=1}^n\sigma_{jm}b_{imL}=0$ for all $j$ or, more compactly,  $\boldsymbol{\Sigma}\mathbf{b}_{i\cdot -1}=\mathbf{0}=\boldsymbol{\Sigma}\mathbf{b}_{i\cdot L}$, where $\mathbf{b}_{i\cdot-1}:=(b_{i1,-1},b_{i2,-1},...,b_{in,-1})'$ and $\mathbf{b}_{i\cdot L}:=(b_{i1L},b_{i2L},...,b_{inL})'$. Since $\boldsymbol{\Sigma}$ has full rank we infer $\mathbf{b}_{i\cdot-1}=\mathbf{0}=\mathbf{b}_{i\cdot L}$ for all $i$, as claimed.  
The eigenvalues of $\boldsymbol{\nu}_i$ are $\tilde{\sigma}_j(2\lambda_{k}-\nu_i)$, $1\leq k\leq L$, $1\leq j\leq n$. We note that if $\mathbf{b}_i$ is a solution of the SSA-problem, then $\nu_i/2$ cannot be an eigenvalue of $\mathbf{M}$. Otherwise, assuming $\nu_i/2=\lambda_{k_0}$ would imply that $\boldsymbol{\nu}_i$ would   map the eigenvectors $\tilde{\mathbf{v}}_{k_0j}$, $j=1,...,n$, in the spectral decomposition of $\mathbf{b}_i$ to zero in \ref{diff_non_hom_matrix_mult}. Consider then the left-hand side of equation \ref{diff_non_hom_matrix_mult}
\[
...
Therefore we can assume that $\nu_i\in \mathbb{R}-\{2\lambda_k|k=1,...,L\}$, that $\boldsymbol{\nu}_i^{-1}$ exists and that
\[
...
\end{eqnarray}
where we inserted \ref{specdec_mult} and where  $\tilde{w}_{ik}=\sqrt{\sum_{j=1}^n w_{ikj}^2}$ and $\mathbf{\tilde{\tilde{v}}}_{ik}:=\sum_{j=1}^n w_{ikj}\mathbf{\tilde{v}}_{kj}/|\tilde{w}_{ik}|$, $k=1,...,L$ are an orthonormal basis of an $L$-dimensional subspace $\tilde{\mathbb{R}}_i^{L}$ of $\mathbb{R}^{nL}$, as claimed. Since $\boldsymbol{\nu}_i$ has full rank, the solution of the SSA-problem is uniquely determined by $\nu_i$, at least down to arbitrary scaling, hereby completing the proof of assertion \ref{ass1_mult}.\\
A proof of \ref{spec_dec_rho_mult} follows from \ref{specdecb_mult0}, relying on orthonormality of $\tilde{\mathbf{v}}_{kj}$ 
\begin{eqnarray}
\rho_i(\nu_i)&=&\frac{\mathbf{b}_{i}(\nu_i)'\mathbf{\tilde{M}}\mathbf{b}_{i}(\nu_i)}{\mathbf{b}_{i}(\nu_i)'\mathbf{\tilde{I}}\mathbf{b}_{i}(\nu_i)}\\
&=&\displaystyle{\frac{\left(D_i\sum_{k=1}^L\sum_{j=1}^n  \frac{w_{ijk}}{2\lambda_{k }-\nu_i}\mathbf{\tilde{v}}_{kj}\right)'\mathbf{\tilde{M}}\left(D_i\sum_{k=1}^L\sum_{j=1}^n  \frac{w_{ijk}}{2\lambda_{k }-\nu_i}\mathbf{\tilde{v}}_{kj}\right)}{\left(D_i\sum_{k=1}^L\sum_{j=1}^n  \frac{w_{ijk}}{2\lambda_{k }-\nu_i}\mathbf{\tilde{v}}_{kj}\right)'\mathbf{\tilde{I}}\left(D_i\sum_{k=1}^L\sum_{j=1}^n  \frac{w_{ijk}}{2\lambda_{k }-\nu_i}\mathbf{\tilde{v}}_{kj}\right)}}\nonumber\\
...
@
We now proceed to assertion \ref{ass4_mult} by showing that the parameter $\nu_i$ is determined uniquely by $\rho_i$ in the holding-time constraint if $|\nu_i|>2\rho_{max}(L)$. Note that all eigenvalues $\tilde{\sigma}_j(2\lambda_{k}-\nu_i)$ of $\boldsymbol{\nu}_i$ must be (strictly) negative, if $\nu_i>2\rho_{max}(L)$, or strictly positive, if $\nu_i<-2\rho_{max}(L)$, so that all eigenvalues of $\boldsymbol{\nu}_i^{-1}$, being the reciprocals of the former, must be of the same sign, either  all positive or all negative. Setting $\tilde{w}_{ik}^2:=\sum_{j=1}^{n}\tilde{\sigma}_jw_{ikj}^2$ in \ref{autocorr_mult} we then obtain
\begin{eqnarray}
...
\end{eqnarray*}
where strict inequality follows from $|\nu_i|>2\rho_{max}(L)\geq 2|\lambda_k|$, so that the product in the denominator is positive, from pairwise difference of $\lambda_k$ and from $\tilde{w}_{ik}^2=\sum_{j=1}^{n}\tilde{\sigma}_jw_{ikj}^2>0$: indeed, $w_{ikj}^2>0$ for all $k,j$ (spectral completeness) and $\tilde{\sigma}_j>0$. We then infer that \ref{all_sum} must be strictly negative so that 
\[
\frac{\partial}{\partial\nu_i}\rho\Big(y_i(\nu_i),y_i(\nu_i),1\Big)<0
\]
...
\boldsymbol{\epsilon}&=&(\boldsymbol{\epsilon}_{1}',...,\boldsymbol{\epsilon}_{1}')'\\
\end{eqnarray*}
would lead to Kroneckerproducts $\mathbf{M}\otimes\boldsymbol{\Sigma}$ and $\mathbf{I}\otimes\boldsymbol{\Sigma}$ and the proof would remain unaffected. %while the arbitrary scaling term is the same across all SSA sub-filters $\mathbf{b}_{ij}$, $j=1,...,n$ of $\mathbf{b}_i$ for fixed $i$, the scale of $\mathbf{b}_i$ could change across targets $z_{it}$, $i=1,...,n$. 
Furthermore, we did not treat the  case of a target $\boldsymbol{\gamma}_{i\cdot\delta}$ with incomplete spectral support mainly because the corresponding singular case is irrelevant for typical applications. Wildi (2023 b) derives a formal solution for univariate prediction problems in the singular case. Lastly, in typical applications one can assume $|\nu_i|>2$ so that $|\nu_i|>2\rho_{max}(L)$, see Wildi (2023 b) for background\footnote{The case $|\nu_i|\leq 2$ would imply that the coefficients of $\mathbf{b}_i$ do not decay towards zero with increasing lag which suggests evidence of an ill-posed estimation problem, see Wildi (2023 b).}. The resulting monotonicity of the lag-one acf $\rho(y_i(\nu_i),y_1(\nu_i),1)$ as a function of $\nu_i$, facilitates numerical computations, as proposed in the following corollary.%Also, \ref{ssa_mult} suggests that the mutivariate SSA-filter assigns equal weight $\frac{1}{(2\lambda_{k}-\nu_i)}$ to the slicing or aggregate $\sum_{j=1}^n w_{ikj}\mathbf{\tilde{v}}_{kj}$ across spectral-weights of the vectors $\mathbf{\tilde{v}}_{kj}$ corresponding to $\boldsymbol{\epsilon}_{ij\cdot}=(\epsilon_{i1\cdot},...,\epsilon_{in\cdot})$. Finally, the orthormal basis $\mathbf{\tilde{v}}_{ik}$ and its adjoined subspace $\tilde{\mathbb{R}}_i^{L}$ of $\mathbb{R}^{nL}$ depend on $i$.\\
...
Let the assumptions of theorem \ref{lambda_mult} hold and assume $|\nu_i|>2$. Then the solution to the SSA-optimization problem \ref{crit1} is 
\begin{equation}\label{prop_sol_un_unc_fast}
\mathbf{b}_{i}(\nu_{0i})=\textrm{sign}_{\nu_{0i}}\sum_{k=1}^L\frac{1}{(2\lambda_{k}-\nu_{0i})}\tilde{w}_{ik}\mathbf{\tilde{\tilde{v}}}_{ik}
\end{equation}
where $\nu_{0i}$ is the unique solution of the non-linear equation
\begin{eqnarray}\label{uni_unco_min}
\frac{\mathbf{b_i(\nu_{0i})}'\mathbf{\tilde{M}}\mathbf{b_i(\nu_{0i})}}{\mathbf{b_i(\nu_{0i})}'\mathbf{\tilde{I}}\mathbf{b_i(\nu_{0i})}}=\rho_i
\end{eqnarray}
The sign $\textrm{sign}_{\nu_{0i}}=\pm 1$ is selected such that $\mathbf{b_i(\nu_{0i})}'\mathbf{\tilde{I}}\boldsymbol{\gamma}_{i\cdot\delta}> 0$ (positive criterion value). 
\end{Corollary}



A proof follows readily from assertions \ref{ass1}-\ref{ass4} of theorem \ref{lambda_mult}, noting that $|\nu_i|>2$ implies $\nu_i>2\rho_{max}(L)$ which ensures uniqueness. In this case, numerical computations are swift due to strict monotonicity. If $|\nu_i|\leq 2\rho_{max}(L)$ then strict monotonicity of the lag-one acf and uniqueness of the solution of \ref{uni_unco_min} are lost, see Wildi (2023 b) for a fleshed-out example. To conclude, the following corollary derives the distribution of the SSA-predictor.  
...
\begin{eqnarray*}
{\boldsymbol{\mu}}_{\mathbf{b}_i}&=&D_i\boldsymbol{\nu}_i^{-1}{\boldsymbol{\mu}}_{\gamma_{i\delta}}\\
{\boldsymbol{\Sigma}}_{\mathbf{b}_i}&=&D_i^2\boldsymbol{\nu}_i^{-1}{\boldsymbol{\Sigma}}_{\gamma_{i\delta}}\boldsymbol{\nu}_i^{-1}
\end{eqnarray*}
If $\hat{\boldsymbol{\gamma}}_{i\cdot\delta}$ is Gaussian distributed then so is $\hat{\mathbf{b}}_i$. 
\end{Corollary}
The proof readily follows from \ref{diff_non_home}. Note that mean, variance and (asymptotic) distribution of the MSE-estimate under various assumptions about $\mathbf{x}_t$ are derived in standard textbooks, see e.g. Brockwell and Davis (1993).
...
Our examples in this section address univariate forecast and nowcast applications: a simple introductory forecast exercise is proposed in section \ref{one_step_fore}; section \ref{var1} considers an extension to a VAR(2)-process and section \ref{sign_het} considers a nowcast signal extraction framework. ???SSA-solutions are based on corollary \ref{lambda_num_gen}, by search of $\lambda\in G$ where $G\subset]-1,1[-\{0\}$ is a finite set of size 1000 %\footnote{Finer resolutions are rarely useful in typical 'non-pathological' applications.} 
of equidistant grid-points and $\nu:=\lambda+1/\lambda$. The solution $\nu_0$ is such that the absolute error in \ref{uni_unco_min} is minimized on the grid $G$. All empirical examples are reproducible in an open source R-package (include link to Github).
...
SSA_obj<-SSA_func(L_short,delta,grid_size,gamma_target,rho0,with_negative_lambda,xi,lower_limit_nu,Sigma,symmetric_target)
...
SSA_obj<-SSA_func(L_long,delta,grid_size,gamma_target,rho0,with_negative_lambda,xi,lower_limit_nu,Sigma,symmetric_target)
...
SSA_obj<-SSA_func(L,delta,grid_size,gamma_target,rho0,with_negative_lambda,xi,lower_limit_nu,Sigma,symmetric_target)
...
\[\left(\begin{array}{c}z_{1t}\\z_{2t}\end{array}\right)=\left(\begin{array}{cc}\Sexpr{A[1,1]}&\Sexpr{A[1,2]}\\\Sexpr{A[2,1]}&\Sexpr{A[2,2]}\end{array}\right)\left(\begin{array}{c}z_{1t-1}\\z_{2t-1}\end{array}\right)+\left(\begin{array}{c}\epsilon_{1t}\\\epsilon_{2t}\end{array}\right)~~~,~~~\Sigma=\left(\begin{array}{cc}\Sexpr{round(Sigma[1,1],2)}&\Sexpr{round(Sigma[1,2],2)}\\\Sexpr{round(Sigma[2,1],2)}&\Sexpr{round(Sigma[2,2],2)}\end{array}\right)\]
where $\Sigma$ is the variance-covariance matrix of $\boldsymbol{\epsilon}_t=(\epsilon_{1t},\epsilon_{2t})'$. We compute a one-step ahead ($\delta=1$) SSA-predictor with holding-times  $ht_1=\Sexpr{ht_vec[1]}$, $ht_2=\Sexpr{ht_vec[2]}$. Weights of SSA- and MSE-predictors are displayed in fig.\ref{filt_coef_var1}: the plotted coefficients correspond to the Wold-decomposition of $\mathbf{z}_t$ i.e. they are applied to $\boldsymbol{\epsilon}_{t-k}$, $k\geq 0$. Performances in terms of theoretical as well as empirical holding-times and criterion-values  are summarized in table \ref{perf_var1}. Empirical measures rely on a sample of length \Sexpr{as.integer(len)} of Gaussian random variables; theoretical numbers are based on proposition \ref{ht_formula} or the criterion in \ref{crit1_mult}\footnote{Note that criterion values of the MSE-predictors are trivially one since the criterion measures correlations with the MSE-benchmark, recall proposition \ref{sa_crit_mult}.}. The table illustrates that the SSA-predictor of the first target $z_{1t+\delta}$ has a shorter holding-time than the MSE-predictor: the predictor 'unsmooths' the target while performing forecasting and the corresponding predictor-weights (top-left panel in fig.\ref{filt_coef_var1})  are overlaid with high-frequency ripples. The SSA-predictor of the second target $z_{2t+\delta}$ has a longer holding-time than the MSE-design: the predictor 'smooths' the target while performing forecasting and its coefficients retain elements of the MSE-pattern but are otherwise 'lifted away' from the zero line (stronger lag-one acf).     
<<label=init,echo=FALSE,results=hide>>=
file = "filt_coef_var1.pdf"
pdf(file = paste(path.out,file,sep=""), paper = "special", width = 6, height = 6)

mplot<-scale(cbind(bk_mat[1:L,1],bk_mat[L+1:L,1]),scale=T,center=F)
...
mplot<-scale(cbind(bk_mat[1:L,2],bk_mat[L+1:L,2]),scale=T,center=F)
colnames(mplot)<-c("Series 1","Series 2")
...
mplot<-scale(mse_mat,scale=T,center=F)
colnames(mplot)<-c("Series 1","Series 2")
colo<-c("blue","red","violet","black","green")
...
\section{Elements of Signal Extraction: Multivariate Process with Heteroclite Targets}\label{sign_het}
...
mplot<-scale(cbind(bk_x_mat[1:L,1],bk_x_mat[L+1:L,1],bk_x_mat[2*L+1:L,1]),scale=T,center=F)
...
mplot<-scale(cbind(bk_x_mat[1:L,2],bk_x_mat[L+1:L,2],bk_x_mat[2*L+1:L,2]),scale=T,center=F)
colnames(mplot)<-c("Series 1","Series 2","Series 3")
...
mplot<-scale(cbind(bk_x_mat[1:L,3],bk_x_mat[L+1:L,3],bk_x_mat[2*L+1:L,3]),scale=T,center=F)
colnames(mplot)<-c("Series 1","Series 2","Series 3")
...
mplot<-scale(cbind(bk_mat[1:L,1],bk_mat[L+1:L,1],bk_mat[2*L+1:L,1]),scale=T,center=F)
colnames(mplot)<-c("Series 1","Series 2","Series 3")
colo<-c("blue","red","green","violet","black")
plot(mplot[,1],main="SSA: first target",axes=F,type="l",xlab="Lag-structure",ylab="filter-weights",ylim=c(min(mplot),max(na.exclude(mplot))),col=colo[1],lwd=1)
mtext(colnames(mplot)[1],col=colo[1],line=-1)
...
mplot<-scale(cbind(bk_mat[1:L,2],bk_mat[L+1:L,2],bk_mat[2*L+1:L,2]),scale=T,center=F)
colnames(mplot)<-c("Series 1","Series 2","Series 3")
...
mplot<-scale(cbind(bk_mat[1:L,3],bk_mat[L+1:L,3],bk_mat[2*L+1:L,3]),scale=T,center=F)
colnames(mplot)<-c("Series 1","Series 2","Series 3")
...
\[\mathbf{z}_t=\sum_{|k|<\infty}\boldsymbol{\Gamma}_k\mathbf{x}_{t-k}
\]
Trends and cycles are typically obtained from lowpass- and bandpass-filters. Often,  $(\mathbf{\Gamma})_{|k|<\infty}$ is an a-causal bi-infinite symmetric design i.e. $\mathbf{\Gamma}_{k}=\mathbf{\Gamma}_{-k}$. For an a-causal signal extraction filter, the estimation of the signal $z_t$ at the current end-point $t=T$ of a finite sample can amount to a challenging prediction exercise, since the left tail $\sum_{k<0}\boldsymbol{\Gamma}_k\mathbf{x}_{T-k}$ is not observed, yet. The corresponding estimation problem, commonly referred to as nowcast, is relevant for assessing the \emph{current} 'state' of a particular phenomenon: for example recession vs. expansion, bear- vs. bull-market, down- vs. up-swing or current seasonally adjusted figure. %Nowcasting can be a challenging estimation problem, in particular when the left-tail $\mathbf{\Gamma}_{-k}$  of the acausal filter decays slowly towards zero for increasing lead $-k$. 
In some cases, the classic MSE-approach can lead to so-called concurrent or real-time estimates which generate unduly many 'false alarms' or noisy zero-crossings. Therefore, a control of the frequency of zero-crossings by the holding-time constraint can be successful in reducing the rate of noisy crossings, see Wildi (2023a) for an application to (univariate) business-cycle analysis. % based on monthly macro-indicators, the industrial production indices of various countries with long and consistent cycle histories.
...
=\sum_{k=-\infty}^{\infty}\boldsymbol{\Gamma}_k\mathbf{x}_t\\
\mathbf{x}_t=\mathbf{A}\mathbf{x}_{t-1}+\boldsymbol{\epsilon_t}
\end{eqnarray*}
...
\Sexpr{A[3,1]}&\Sexpr{A[3,2]}&\Sexpr{A[3,3]}\end{array}\right)$ and 
\begin{eqnarray*}
%a11<-0.7
%a131<-1.5
%a132<--0.8
...
\end{eqnarray*}
where $\xi_{13|k|},\xi_{23|k|}$ and $\xi_{33|k|}$ are the coefficients of the MA-inversions of the AR(2)-filters with parameters $(\Sexpr{a131},\Sexpr{a132})$, $(\Sexpr{a231},\Sexpr{a232})$ and  $(\Sexpr{a331},\Sexpr{a332})$ respectively.  In applications, filters can be specified either in a more or less 'ad hoc' way (for example Hoddrick-Prescott or Henderson or otherwise interesting or meaningful MA-designs) or in terms of optimal model-based designs see e.g. McElroy and Wildi (2020). Either way, we here do not preclude this choice and propose a mix of 'heteroclite' lowpass designs chiefly emphasizing the flexibility of our approach. Concerning the target we aim at a nowcast of $\mathbf{z}_{t+\delta}$ with $\delta=0$, imposing holding-time constraints $ht_1=\Sexpr{ht_vec[1]}$, $ht_2=\Sexpr{ht_vec[2]}$ and $ht_3=\Sexpr{ht_vec[3]}$ for the SSA-nowcasters of $z_{1t+1},z_{2t+1}$ and $z_{3t+1}$. Furthermore, we derive nowcast-weights $ (\mathbf{b}\cdot\boldsymbol{\xi})_{i\delta}$, as assigned to $\boldsymbol{\epsilon}_{it}$ (convolution), or $\mathbf{b}_i$ as applied to $\mathbf{x}_{it}$ (deconvolution), see section \ref{ext_stat}. Fig.\ref{filt_coef_signal_extraction_heteroclite} displays all filters: SSA as applied to $\mathbf{x}_t$ (top panels) and to $\boldsymbol{\epsilon}_t$ (mid panels) and MSE as applied to $\boldsymbol{\epsilon}_t$ (bottom panels). From left to right the nowcasters target $z_{1t}$, $z_{2t}$ and $z_{3t}$ while the colors distinguish filters as applied to $\epsilon_{1t-k}$ or $x_{1t-k}$ (\Sexpr{colo[1]}), $\epsilon_{2t-k}$ or $x_{2t-k}$ (\Sexpr{colo[2]}) and $\epsilon_{3t-k}$ or $x_{3t-k}$ (\Sexpr{colo[3]}). Empirical sample performances of the filters are summarized in table \ref{perf_signal_extraction_heteroclite_sample}. A comparison with expected (true) numbers in table \ref{perf_signal_extraction_heteroclite_true} confirms pertinence of the approach: cross-correlations and lag-one acf expressions are based on \ref{gen_stat_x}, sign-accuracies P$\Big(\sign(z_{it+\delta})=\sign(y_{it})\Big)$ rely on \ref{arcsin} and expected holding-times are derived from \ref{ht}, inserting the correspondent lag-one acfs. %A comparison of holding-times suggests that all SSA-designs emphasize smoothness in this example. 
...
The effect of imposing a markedly larger holding-time of SSA, when compared to MSE, consists in pulling filter-weights away from the zero-line  while taking over some of the main-patterns of MSE, see middle column, middle and bottom graphs in fig.\ref{filt_coef_signal_extraction_heteroclite}. Similar holding-times leads to a very tight fit of SSA- and MSE-nowcasts (last row in table \ref{perf_signal_extraction_heteroclite_sample}). Note that the SSA-solution is invariant to the choice of the criteria in the first three columns of the table: all three functions of the SSA-filter are equivalent objectives in optimization terms.
<<label=z_box_plot_pure_mba_2.pdf,echo=FALSE,results=tex>>=
file = "filt_coef_signal_extraction_heteroclite.pdf"
cat("\\begin{figure}[H]")
cat("\\begin{center}")
cat("\\includegraphics[height=3in, width=6in]{", file, "}\n",sep = "")
cat("\\caption{SSA applied to $x_t$ (deconvolution: top graphs), SSA applied to $\\epsilon_t$ (convolution, middle graphs) and MSE- applied to $\\epsilon_t$ (bottom). Nowcasters of the first target series $z_{1t}$ (left), second target $z_{2t}$ (middle) and third target $z_{3t}$ (right). Filter-weights  applied to $x_{1t}$ or $\\epsilon_{1t}$ (blue) $x_{2t}$ or $\\epsilon_{2t}$ (red) and $x_{3t}$ or $\\epsilon_{3t}$ (green)", sep = "")
...
xtable(round(perf_mat,2), dec = 1,digits=2,
paste("Expected (true) performances of SSA-nowcasts: Sign accuracy ${P}\\Big(\\sign(z_{it+\\delta})=\\sign(y_{it})\\Big)$, correlation with target ${\\rho}(y_i,z_i,\\delta)$, correlation with MSE ${\\rho}(y_i,\\hat{z}_{i\\delta})$ and holding-time $ht(y_i|\\mathbf{b}_i)=\\frac{\\pi}{\\arccos(\\rho(y_i,y_i,\\delta))}$ with $\\delta=0$ (nowcast). Expected (true) holding-time of MSE in last Column."),
...
\subsection{Non-Gaussian Data}\label{non_gaussian}
