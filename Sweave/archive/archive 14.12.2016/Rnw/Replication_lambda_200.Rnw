...
In order to fit the data,  Morley, Nelson, and Zivot (2003), \href{https://www.dropbox.com/s/rvjgddwdjsjjig4/mnz03.pdf?dl=0}{MNZ (2003)}, for short, propose the following Unobserved-Components (UC-) model
\begin{eqnarray}
...
\begin{itemize}
\item Jan-1947 to Feb-1998: replicate results in \href{https://www.dropbox.com/s/rvjgddwdjsjjig4/mnz03.pdf?dl=0}{MNZ (2003)}
...
In order to illustrate handling of the $dlm$-package we attempt to replicate results in \href{https://www.dropbox.com/s/rvjgddwdjsjjig4/mnz03.pdf?dl=0}{MNZ (2003)}. 
...
@
The optimization affected slightly our initial estimates and therefore the final estimates in table \ref{z_ss_uc0_t} do not coincide with those reported in \href{https://www.dropbox.com/s/rvjgddwdjsjjig4/mnz03.pdf?dl=0}{MNZ (2003)}, table 1.
...
<<label=z_us_real_log_gdp_comp.pdf,echo=FALSE,results=tex>>=
  ...
  cat("\\end{figure}")
@
The cycle is similar, but not identical, to the estimate in \href{https://www.dropbox.com/s/rvjgddwdjsjjig4/mnz03.pdf?dl=0}{MNZ (2003)}, fig.1. 
\end{enumerate}
...
lgdp_07 <- ts(100*log(mydata),start=start_year,frequency=4)
@
<<echo=True>>=
# Data up to Dec 2014
end_date<-"2014-11-30"
...
# Load real gdp
mydata<-Quandl(c("FRED/GDPC96"),start_date=start_date,end_date=end_date,type="xts")
lgdp_14 <- ts(100*log(mydata),start=start_year,frequency=4)
@
\item Estimation:
<<echo=False>>=
...
\item Cycle-estimates in fig.\ref{z_us_real_log_gdp_comp_i2} look similar. Counting-in the I(1)-model in the previous section, this observation may lead one to conclude that neither the integration-order nor the `implied' cycle-length are clearly identified by the data. To some extent, the final decision rests up to the analyst's preference(s). 
\item Fig.\ref{z_us_real_log_gdp_comp_i2} suggests that drift and cycle estimates correlate, against the fact that the model assumes orthogonality of components.  
\end{itemize}
...
Once replicated by the DFA, the trend-cycle model could be customized. If time and interest allow...
...
\href{https://www.dropbox.com/s/21trk2pcgpyrtkl/mhpfilter.pdf?dl=0}{Maravall and Kaiser (2004)} and 
\href{https://www.dropbox.com/s/smfj7zvowbita7a/SSRN-id1100238.pdf?dl=0}{McElroy (2008)}. In this framework the HP-trend $y_t$ is the output of a symmetric filter\footnote{We here assume that $y_t$ is estimated in the middle of a large sample, such that symmetric filters can be applied.}:
\[y_t=\frac{q}{q+(1-B)^2(1-F)^2}x_t\]
where $q=1/\lambda_{HP}$ and where $F=B^{-1}$ is the forward operator. If
\begin{eqnarray*}
x_t&=&T_t+\lambda_{HP} I_t\\
(1-B)^2T_t&=&\nu_t
\end{eqnarray*}
where $I_t,\nu_t$ are mutually independent Gaussian white noise sequences with identical variances,  
then $y_t$ is an optimal MSE estimate of the unobserved trend $T_t$. Note that $\lambda_{HP}$ can be interpreted as an inverse SNR: for $\lambda_{HP}=0$ the noise vanishes and therefore $x_t=T_t$ or, equivalently, $y_t=x_t$. For $\lambda_{HP}=1600$ the DGP of $x_t$ is found to be 
\begin{eqnarray}\label{implicit_mode_ass}
(1-B)^2x_t=(1-1.7771B+0.7994B^2)\epsilon_t
\end{eqnarray}
where $\epsilon_t$ is Gaussian white noise, see \href{https://www.dropbox.com/s/21trk2pcgpyrtkl/mhpfilter.pdf?dl=0}{Maravall and Kaiser (2004)} and \href{https://www.dropbox.com/s/smfj7zvowbita7a/SSRN-id1100238.pdf?dl=0}{McElroy (2008)}\footnote{The latter author provides R-code for an exact derivation of the MA-parameters as a function of $\lambda_{HP}$, see section \ref{hp_exe_repli} below.}. The corresponding implicit pseudo-spectral density is
...
nobs <- length(lgdp)
@
\item Load the R-function for deriving the MA-coefficients of the implicit time series model of the HP-filter, see \href{https://www.dropbox.com/s/smfj7zvowbita7a/SSRN-id1100238.pdf?dl=0}{McElroy (2008)}.
<<echo=True>>=
...
weight_func_h<-abs((1-1.77709*exp(1.i*(0:(K))*pi/(K))+
                      0.79944*exp(1.i*2*(0:(K))*pi/(K)))/
...
imdfa_hp<-mdfa_analytic(K,L,lambda,weight_func_hp,Lag,Gamma,eta,cutoff,
                        i1,i2,weight_constraint,lambda_cross,lambda_decay,
                        lambda_smooth,lin_eta,shift_constraint,grand_mean,
                        b0_H0,c_eta,weights_only=F,weight_structure,
                        white_noise,synchronicity,lag_mat)
...
plot_title<-"Replication HP-real-time by DFA: Lag 0-240"
...
rownames(mplot)<-paste("Lag ",0:(nrow(mplot)-1))
colnames(mplot)<-c("Replication by DFA","HP-real-time")
plot_title<-"Replication HP-real-time by DFA: Lag 0-20"
freq_axe<-rownames(mplot)
title_more<-c("DFA","HP")
mplot_func(mplot,freq_axe,plot_title,title_more,insamp,colo)
invisible(dev.off())
...
print(paste("Transfer function in frequency zero: ",sum(imdfa_hp$b),sep=""))
# Check second-order: time-shift should vanish
print(paste("Time-shift in frequency zero: ",(1:(L-1))%*%imdfa_hp$b[2:L],sep=""))
@
\end{enumerate}
...
\]
where ${\gamma}_k^{HP-Trend,1600}$ are the coefficients of the symmetric HP-trend filter with $\lambda_{HP}=1600$\footnote{Note that the coefficients as computed by the function $hpfilter$ ($mFilter$-package) correspond to the gap. The trend coefficients in the previous section were obtained by the transformation $\gamma_k^{trend}=1-\gamma_k^{gap}$.}. The resulting time series $\textrm{gap}_t$ is likely to be positive during expansions and negative during contractions. Although these dynamic characteristcs alude to a `cycle', the target filter 
\[\Gamma^{gap}(\omega):=1-\Gamma^{trend}(\omega)\]
is not a bandpass but a highpass instead, see fig.\ref{z_HP_us_real_log_gdp_hp_diff__gap_amp}, red line. The highpass has a double zero in the unit-root frequency zero: it transforms a non-stationary I(2)-process in a stationary component. The real-time (one-sided) filter inherits this property, too.  
...
\begin{eqnarray}
&&\frac{2\pi}{T}\sum_{k=-[T/2]}^{[T/2]}\left|\Gamma(\omega_k)^{HP-trend}(1-\exp(-i\omega_k))-\hat{\Gamma}(\omega_k)(1-\exp(-i\omega_k)) \right|^2 h(\omega_k)\nonumber\\
&=&\frac{2\pi}{T}\sum_{k=-[T/2]}^{[T/2]}\left|\Gamma(\omega_k)-\hat{\Gamma}(\omega_k) \right|^2 \tilde{h}(\omega_k)\to\min_{\mathbf{b}}\label{dfa_ms_hp_cycle}
\end{eqnarray}
where 
\begin{eqnarray}
...
\subsubsection{Exercises: Analyse HP-Gap and HP-Cycle}
...
\end{itemize}
\item Compute the cycle-periodicity, measured in years. Hint: derive the duration from the peak of the amplitude function in fig.\ref{z_HP_us_real_log_gdp_hp_diff__gap_amp}.
<<echo=True>>=
...
lambda_HP<-100
# proceed to filtering
x_200 <- hpfilter(x,type="lambda", freq=lambda_HP)
# Extract the coefficients of the symmetric trend:
parm_200<-diag(rep(1,len))-x_200$fmatrix
# Determine the transfer function
Gamma_200<-0:(K)
for (k in 0:(K))
{
  omegak<-k*pi/(K)
  Gamma_200[k+1]<-parm_200[len/2,len/2]+2*parm_200[(len/2+1):len,len/2]%*%
              cos((1:(len/2))*omegak)
}
# Specify cycle-target 
Gamma_cycle_200<-abs(Gamma_200*(1-exp(1.i*pi*(0:K)/K)))
# Compute cycle-length
peak_frequency_200<-(which(Gamma_cycle_200==max(Gamma_cycle_200))-1)*pi/K
periodicity_in_quarters_200<-round(2*pi/peak_frequency_200,3)
print(paste("Cycle-periodicity in years for lambda=",lambda,": ",
            periodicity_in_quarters_200/4,sep=""))  
@
The mean duration of \Sexpr{round(periodicity_in_quarters_l/4,3)} (years) would match business-cycles of US-GDP better. 
\end{enumerate}
...
We work with the specifications derived in the above analysis: HP-gap based on $\lambda_{HP}=1600$ and HP-cycle based on $\lambda_{HP}=200$. In the latter case, $\lambda_{HP}=200$, we derive the new implicit model as well as the corresponding new implicit pseudo-spectrum.
\begin{enumerate}
\item Compute MA(2)-parameters and pseudo-spectrum for $\lambda_{HP}=200$.
<<echo=True>>=
# Select lambda
lambda_hp_200<-100
# Compute MA(2)-coefficients
q<-1/lambda_hp_200
hp_filt_obj<-hpFilt(q,L_hp)
hp_filt_obj$ma_model
# Determine pseudo-spectral density
weight_func_h<-abs((1+hp_filt_obj$ma_model[2]*exp(1.i*(0:(K))*pi/(K))+
                      hp_filt_obj$ma_model[3]*exp(1.i*2*(0:(K))*pi/(K)))/
                      (1-exp(1.i*(0:(K))*pi/(K)))^2)
# Specify (square-root) spectra of target (first column) 
#   and of explanatory variable (second column): target and 
#   explanatory are the same here (univariate filter)
weight_func_hp_200<-cbind(weight_func_h,weight_func_h)
# Remove the singularity in frequency zero 
#   We can assign an arbitrary (finite) value
#   This number will be cancelled anyway by imposing filter constraint(s) 
weight_func_hp_200[1,]<-0
@
\item Compare amplitude functions of target cycles for $\lambda_{HP}=1600$ and $\lambda_{HP}=200$.
<<echo=True>>=
file = paste("z_HP_us_real_log_gdp_hp_cycle_1600_cycle_200.pdf", sep = "")
pdf(file = paste(path.out,file,sep=""), paper = "special", 
    width = 6, height = 6)
insamp<-1.e+99
mplot<-cbind(Gamma_cycle,Gamma_cycle_200)
plot_title<-"Target HP-cycles"
freq_axe<-rep(NA,K+1)
freq_axe[1]<-0
freq_axe[1+(1:6)*K/6]<-c(paste(c("",2:5),"pi/6",sep=""),"pi")
title_more<-NA
...
invisible(dev.off())
@
<<label=z_HP_us_real_log_gdp_hp_cycle_1600_cycle_200.pdf,echo=FALSE,results=tex>>=
  file = paste("z_HP_us_real_log_gdp_hp_cycle_1600_cycle_200", sep = "")
  cat("\\begin{figure}[H]")
  cat("\\begin{center}")
  cat("\\includegraphics[height=3in, width=3in]{", file, "}\n",sep = "")
  cat("\\caption{Amplitude functions of target HP-cycles: lambda=1600 (blue) vs. lambda=200 (red)", sep = "")
  cat("\\label{z_HP_us_real_log_gdp_hp_cycle_1600_cycle_200}}", sep = "")
  cat("\\end{center}")
  cat("\\end{figure}")
@
The peak of the new target signal (red) is shifted to the right, as desired.
\item Compute an optimal (MSE) real-time filter of the target $cycle_t$: apply \ref{dfa_ms_hp_cycle} and set $i1=T, i2=F$ and $L=50$. Note that we apply the \emph{trend}-target to \emph{differenced} GDP-data, which is formally equivalent to applying the cycle-target to GDP-data in levels.
<<echo=True>>=
L<-50
...
weight_func_hp_200_diff<-weight_func_hp_200*
  abs(1-exp(1.i*(0:(nrow(weight_func_hp_200)-1)*pi/(nrow(weight_func_hp_200)-1))))
# Set default settings for MDFA (MSE, no regularization)
source(file=paste(path_MDFA.pgm,"control_default.r",sep=""))
...
#   The target is the trend (not the cycle)
imdfa_hp_cycle_200<-mdfa_analytic(K,L,lambda,weight_func_hp_200_diff,Lag,Gamma,
                        eta,cutoff,i1,i2,weight_constraint,lambda_cross,
                        lambda_decay,lambda_smooth,lin_eta,shift_constraint,
...
print(paste("Transfer function in frequency zero: ",
            sum(imdfa_hp_cycle_200$b),sep=""))
# Check second-order: this is not imposed because we work with
#   differenced data
print(paste("Time-shift in frequency zero: ",
            round((1:(L-1))%*%imdfa_hp_cycle_200$b[2:L],3),sep=""))
@
As expected, the first-order level constraint is met but the time-shift in frequency zero is now \Sexpr{round(sum((1:(L-1))*imdfa_hp_cycle_200$b[2:L]),3)}. 
...
<<echo=True>>=
x_cycle_200<-diff(x)
gamma_cycle_200<-as.vector(imdfa_hp_cycle_200$b)
# Coefficients should add to one: trend filter applied to 
#   differenced data
sum(gamma_cycle_200)
for (j in L:length(x_cycle))
  x_cycle_200[j] <- gamma_cycle_200%*%diff(x)[j:(j-(L-1))]
x_cycle_200[1:(L-1)]<-NA
@
\end{itemize}
\item Compare the proposed real-time cycle estimates: standardize both series for ease of visual inspection, see fig.\ref{z_HP_us_real_log_gdp_hp_bp}.
<<echo=True>>=
...
#     shifted/adjusted
mploth<-cbind(cbind(c(NA,diff(x)),x_gap,x_cycle_200))
# Standardization
mplot<-scale(mploth)  
plot_title<-"HP-Gap (red) vs. HP-Cycle (blue)"
...
imdfa_hp_cust_200<-mdfa_analytic(K,L,lambda,weight_func_hp_200_diff,Lag,Gamma,eta,cutoff,
                        i1,i2,weight_constraint,lambda_cross,lambda_decay,
                        lambda_smooth,lin_eta,shift_constraint,grand_mean,
                        b0_H0,c_eta,weights_only=F,weight_structure,
                        white_noise,synchronicity,lag_mat)
...
title_more<-NA
mplot<-cbind(imdfa_hp_cust_200$b,imdfa_hp_cycle_200$b)
...
@
<<echo=True>>=
x_cycle_cust_200<-diff(x)
gamma_cust_200<-imdfa_hp_cust_200$b[1:L]
for (j in L:length(x_cycle_cust_200))
  x_cycle_cust_200[j] <- gamma_cust_200%*%diff(x)[j:(j-(L-1))]
x_cycle_cust_200[1:(L-1)]<-NA
...
insamp<-1.e+99
colo<-c("red","blue")
mplot<-scale(cbind(x_cycle_200,x_cycle_cust_200))
...
lines(mplot[,1],col="blue")  #ts.plot(x_cycle)  ts.plot(mplot[,2])
invisible(dev.off())
@
...
<<label=z_CF_us_real_log_gdp_fb.pdf,echo=FALSE,results=tex>>=
  ...
  cat("\\end{figure}")
@
Real-time model-based and DFA filters now match at all lags\footnote{Minor (negligible) deviations are due to the finite resolution of the frequency-grid.}.
...
imdfa_cf_L<-mdfa_analytic(K,L,lambda,weight_func_cf,Lag,Gamma_cf,eta,cutoff,
                        i1,i2,weight_constraint,lambda_cross,lambda_decay,
                        lambda_smooth,lin_eta,shift_constraint,grand_mean,
                        b0_H0,c_eta,weights_only=F,weight_structure,
                        white_noise,synchronicity,lag_mat)
...
\item The customization triplet ($Lag,\lambda,\eta$) usefully extends the action of the ATS-trilemma to arbitrary  real-valued leads, $Lag<0$, or lags, $Lag>0$. A corresponding application to the Christiano Fitzgerald filter illustrates the proceeding when designing a business-cycle indicator based on US-GDP.
\end{itemize}
