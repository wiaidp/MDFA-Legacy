...
\item Verify that filter coefficients of the bivariate design are optimal: this is left as an exercise to the reader (any other set of bivariate filter coefficients, of length \Sexpr{L}, should increase the criterion value \Sexpr{round(mdfa_obj$rever,3)} and/or the sample MSE \Sexpr{round(perf_mse[1],3)}).
